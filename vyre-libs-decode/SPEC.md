...
Layer `libraries`. Owner `libs-decode`.

## Owns

Data decoding and decompression compositions: base64, hex, DEFLATE, and the encodex container format.
...
5. A parser for a programming language. Grammar-driven parsing is `vyre-libs-parsing`.
6. A container format that names a model convention. Decoding produces bytes and typed extents, not tensors with meanings.
...
- `vyre-libs-text` over the `libs-text` seam.
...
`vyre-foundation`, `vyre-primitives` and `vyre-spec` must never depend on `vyre-libs-decode`. The edge is one way: a cycle back into this crate makes the two crates one crate that cannot be built, reviewed or published apart.
...
- The public surface is recorded in `docs/public-api/vyre-libs-decode.txt`. An item added, removed or moved changes that file in the same change.
