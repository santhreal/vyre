...
Layer `emitter`. Owner `metal-emitter`.

## Owns

Consume verified lowering products and emit native Apple shader source through
the shared emitter path.
...
- `vyre-driver-metal` over the `metal-emitter` seam, private.

## Direction that may not reverse

`vyre-emit-naga`, `vyre-foundation`, `vyre-lower` must never depend on
`vyre-emit-metal`. The edge is one way: a cycle back into this crate makes the
...
- The crate declares no feature beyond `default`, so every build of it is the
  same build.
- The public surface is recorded in `docs/public-api/vyre-emit-metal.txt`. An
