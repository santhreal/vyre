...
use crate::naming::{
    CrateRoot, PUBLIC_API_SNAPSHOT_DIR, SOURCE_TREES, directory_stutter_failures,
    generic_module_name_failures, numbered_sibling_failures, sibling_module_failures,
};
...
///
/// An attribute is read only where code can carry one. A raw search for the
/// text matched a doc comment that quotes `#[cfg(test)]` and deleted the
/// production function documented below it, along with every span the aborted
/// search would have found after it: this file documents the scanner it
/// implements, so its own `mod tests` survived the strip and its registration
/// fixtures were read as production registrations. Two operations then had two
/// defining crates and a third had none.
///
/// A malformed attribute is skipped rather than ending the scan, because
/// stopping leaves every later test item in the production text, which is the
/// direction that turns fixtures into contract facts.
fn cfg_test_spans(text: &str) -> Vec<(usize, usize)> {
    const ATTR: &str = "#[cfg(";
    let mut spans: Vec<(usize, usize)> = Vec::new();
    let mut consumed = 0usize;
    for attr_start in code_offsets(text) {
        if attr_start < consumed || !text[attr_start..].starts_with(ATTR) {
            continue;
        }
