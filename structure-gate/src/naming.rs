//! Module layout and file naming rules.
//!
//! A reader finds an item by opening the file its name promises, so these rules
//! judge names and placement rather than contents: a module split across a file
//! and a directory of the same name, a name that states no contract, siblings
//! told apart by a number, and a file repeating the directory holding it.
//!
//! They were written beside the operation-identity rules in the crate root,
//! which grew past the line cap this workspace holds every file to. The rules
//! and the tests that pin them live here.
...
/// These rules read file paths, and `judged_name_of`, `binary_name_of`,
/// `module_path_of` and the banned-name table are crate-private, so no
/// integration test can reach them to pin the boundary each one draws. What the
/// rules answer over the real checkout is asserted in
/// `tests/crate_structure_contracts.rs`.
#[cfg(test)]
mod tests {
    use super::*;
