...
Layer `libraries`. Owner `libs-text`.

## Owns

Text processing compositions: byte classification, UTF-8 validation, and line indexing.
...
5. A locale, a collation table, or a natural-language rule set.
6. A decoder for a compressed or encoded container. That is `vyre-libs-decode`.
...
`vyre-foundation`, `vyre-primitives` and `vyre-spec` must never depend on `vyre-libs-text`. The edge is one way: a cycle back into this crate makes the two crates one crate that cannot be built, reviewed or published apart.
...
- The public surface is recorded in `docs/public-api/vyre-libs-text.txt`. An item added, removed or moved changes that file in the same change.
