...
//! `vyre_libs_solvers::solvers::quantized_dispatch` at run time, so an entry point
    ...
    vec![
        EntryPoint {
            ...
            call: Box::new(|dispatcher| {
                unpack_i4x8_via(dispatcher, &semantic_requests::wrapper_policy(), &[0], 8).map(drop)
            }),
        },
        EntryPoint {
...
/// `vyre-libs-solvers/src/solvers/quantized_dispatch/mod.rs`, read from that file so a new
...
fn published_entry_point_names() -> BTreeSet<String> {
    let path = vyre_test_support::monorepo::vyre_workspace_root()
        .join("vyre-libs-solvers/src/solvers/quantized_dispatch/mod.rs");
...
fn every_published_quantized_entry_point_has_a_readback_contract_row() {
    ...
    assert!(
        missing.is_empty(),
        "Fix: quantized dispatch entry points {missing:?} are published by vyre_libs_solvers::solvers::quantized_dispatch but have no row in this gate. Add a row asserting the backend-readback contract for each."
    );

    let stale = covered.difference(&published).collect::<Vec<_>>();
    assert!(
        stale.is_empty(),
        "Fix: this gate has rows for {stale:?}, which vyre_libs_solvers::solvers::quantized_dispatch no longer publishes. Delete the stale rows."
    );
}
