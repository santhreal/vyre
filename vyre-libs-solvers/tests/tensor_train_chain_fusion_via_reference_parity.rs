...
use vyre_libs_solvers::solvers::tensor_train_chain_fusion::fusion_pressure_via;
...
fn fusion_pressure_via_matches_known_chains() {
    ...
    assert_eq!(
        fusion_pressure_via(&dispatcher, &semantic_requests::wrapper_policy(), &[3, 4]).unwrap(),
        12.0
    );
    assert_eq!(
    ...
    assert_eq!(
        fusion_pressure_via(&dispatcher, &semantic_requests::wrapper_policy(), &[]).unwrap(),
        0.0
    );
}
