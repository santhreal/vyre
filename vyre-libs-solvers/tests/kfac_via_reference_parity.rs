...
//! `ReferenceSemanticExecutor` (real reference-eval of the kernel IR) and asserts the BASIS-INVARIANT
...
//! the old RO-sequential `ReferenceSemanticExecutor` mapped `blocks_in` to the wrong (zero `blocks_out`)
...
use vyre_libs_solvers::solvers::kfac_autotune_step::kfac_autotune_step_via;
