...
use vyre_libs_math::math::mori_zwanzig::mz_project_step;
...
pub fn coarsen_region_state_fixed_via_with_scratch_into(
    ...
    let output = vyre_libs_builder::plumbing::host::dispatch_buffers::execute_program_first_output(
        dispatcher,
        program,
        &scratch.inputs,
        policy,
    ...
    decode_u32_output_exact(&output, n as usize, "coarsen_region_state_fixed_via", out)
}
    ...
    impl SemanticExecutor for MoriDispatcher {
        fn execute(
            ...
            let compute_ordered = || -> Result<Vec<Vec<u8>>, SemanticExecutionError> {
                ...
                let state = vyre_libs_builder::plumbing::host::dispatch_buffers::read_u32s(&inputs[1]);
