...
/// `vyre_libs_math::math::tensor_train::tt_contract_step`'s multiply-then-shift contract without
...
mod tests {
    ...
    fn via_pressure_matches_unit_core_reference() {
        ...
        let ranks = vec![2, 3, 5];
        let pressure =
            fusion_pressure_via(&dispatcher, &vyre_libs_builder::test_parity_oracles::policy(), &ranks)
