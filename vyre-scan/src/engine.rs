...
impl MatchScan for GpuLiteralSet {
    ...
    fn cache_key(&self) -> String {
        // Rendered from `pattern_fingerprint` (literal_set.rs), the single
        // owner of a literal set's identity hash. This used to repeat the
        // hash expression and rely on a test to keep the two in step, which
        // is one definition too many for a value that names cache files.
        //
        // That owner uses vyre's FNV-1a primitive, not std::DefaultHasher:
        // DefaultHasher's SipHash seed is randomized per process, so cache
        // files written by one run would never match keys generated by the
        // next, silently breaking the cache. FNV-1a is deterministic, fast,
        // and an identity hash needs no collision resistance.
        format!("lit-{:016x}", self.pattern_fingerprint())
    }
}
...
impl MatchEngineCache for GpuLiteralSet {
    ...
    // One owner: the wire version lives in literal_set.rs; referencing it here
    // means a future bump cannot desync the cache-version signal.
    const WIRE_VERSION: u32 = super::literal_set::LITERAL_SET_WIRE_VERSION;
...
/// Deterministic FNV-1a over the little-endian bytes of one or more `u32`
/// slices. The single owner for every identity/cache hash over word arrays in
/// the scan crate (cache keys and literal-set fingerprints) so they cannot drift.
pub(crate) fn fnv1a64_word_slices<const N: usize>(slices: [&[u32]; N]) -> u64 {
