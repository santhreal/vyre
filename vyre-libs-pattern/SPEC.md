...
Layer `libraries`. Owner `libs-pattern`.

## Owns

Pattern matching compositions: substring search, DFA and NFA execution, regex scanning pipelines, and bracket matching.
...
5. A regex parser that builds its automaton on the host at dispatch time. The automaton is data the composition consumes.
6. A grammar-driven parse. That is `vyre-libs-parsing`.
...
`vyre-foundation`, `vyre-primitives` and `vyre-spec` must never depend on `vyre-libs-pattern`. The edge is one way: a cycle back into this crate makes the two crates one crate that cannot be built, reviewed or published apart.
...
- The public surface is recorded in `docs/public-api/vyre-libs-pattern.txt`. An item added, removed or moved changes that file in the same change.
