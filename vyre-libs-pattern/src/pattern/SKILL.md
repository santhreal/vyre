...
- `substring_search` (`pattern-substring`): single-pattern brute-force
  match, one invocation per haystack offset.
- `aho_corasick` / `cooperative_dfa_scan` / `dfa_compile` /
  `dfa_compile_with_budget` (`pattern-dfa`): multi-pattern scanners and
  CPU-side Aho-Corasick table builders with size-budget enforcement.
- `ScanProgram` and NFA tables (`pattern-nfa`).
- `compile_regex_set` / `regex_compile` / `RegexDfaPipeline` and related
  admission helpers (`pattern-regex`, often combined with DFA features).
