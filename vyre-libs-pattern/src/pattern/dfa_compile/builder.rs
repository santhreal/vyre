//! Trie construction and failure-link collapsing for multi-pattern DFAs.
...
#[inline]
pub(super) fn fold_ascii_byte(b: usize, case_insensitive: bool) -> usize {
...
pub(super) fn dfa_compile_inner_capped(
    patterns: &[&[u8]],
    state_cap: usize,
    ...
    for state in 0..state_count {
        ...
        for b in 0..256usize {
            ...
            loop {
                let child = trie[s][fb];
                let idx = vyre_libs_builder::builder::state_machine::TableStateMachineComposer::flat_byte_index(
                    state as u32,
                    b as u8,
                );
                if child != NO_TRANSITION {
