...
use vyre_libs_hash::hash::hypervector::{hypervector_majority_bundle, hypervector_xor_bind};
...
fn run_xor_bind(a: &[u32], b: &[u32]) -> Vec<u32> {
    ...
    let outputs = vyre_reference::ReferenceRequest::standard(&program, &[pack(a), pack(b)])
        .outputs()
...
fn run_majority(hvs: &[Vec<u32>], dim_words: u32) -> Vec<u32> {
    ...
    let outputs = vyre_reference::ReferenceRequest::standard(&program, &[pack(&stacked)])
        .outputs()
