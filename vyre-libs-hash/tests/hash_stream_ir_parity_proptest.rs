...
use vyre_libs_hash::hash::adler32::adler32_program;
use vyre_libs_hash::hash::multi_hash::multi_hash_program;
...
fn run_adler(bytes: &[u8]) -> u32 {
    ...
    let outputs =
        vyre_reference::ReferenceRequest::standard(&program, &[pack(&bytes_to_words(bytes))])
            .outputs()
