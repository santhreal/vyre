...
fn linear_tiled_uses_tiled_matmul_kernel_shape() {
    let program = vyre_libs_nn::nn::linear::linear_tiled("x", "w", "b", "out", 37, 65, 16)
...
fn attention_default_maps_one_workgroup_to_a_query_row() {
    ...
    let program = vyre_libs_nn::nn::attention::attention("q", "k", "v", "out", 16, 4);
...
fn softmax_default_uses_tiled_workgroup_scratch() {
    let program = vyre_libs_nn::nn::attention::softmax("input", "output", 513);
...
fn rms_norm_default_uses_tiled_workgroup_scratch() {
    let program = vyre_libs_nn::nn::norm::rms_norm("input", "output", 777, 1.0e-5);
