...
impl RegionDependenceGraph {
    ...
    pub fn from_programs(programs: &[Program]) -> Self {
            ...
            for buf in prog.buffers() {
                ...
                match buf.access() {
                    BufferAccess::ReadOnly | BufferAccess::Uniform => {
                        read_arms_per_buf.entry(name.clone()).or_default().push(arm_idx);
                    ...
                    BufferAccess::ReadWrite | BufferAccess::WriteOnly => {
                        write_arms_per_buf.entry(name.clone()).or_default().push(arm_idx);
        ...
        for i in 0..programs.len() {
            for j in (i + 1)..programs.len() {
                let has_edge = edges.iter().any(|e| (e.producer == i && e.consumer == j) || (e.producer == j && e.consumer == i));
                if !has_edge {
    ...
    #[must_use]
    pub fn earliest_handoff_between(&self, producer: usize, consumer: usize) -> Option<HandoffLocation> {
        self.edges
            .iter()
...
pub fn classify_program_handoff(producer: &Program, consumer: &Program) -> HandoffLocation {
    ...
    let uses_shared = producer.buffers().iter().any(|b| b.access() == BufferAccess::Workgroup)
        || consumer.buffers().iter().any(|b| b.access() == BufferAccess::Workgroup);
    ...
    if !producer.workgroup_size_is_schedule_only() && !consumer.workgroup_size_is_schedule_only() && prod_wg != cons_wg {
...
#[must_use]
pub fn earliest_region_handoff(producer: &LogicalRegion, consumer: &LogicalRegion) -> HandoffLocation {
    ...
    match relation {
        ...
        RegionRelation::ReductionConsumer | RegionRelation::StencilWindow => HandoffLocation::WorkgroupShared,
