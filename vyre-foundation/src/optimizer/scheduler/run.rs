...
impl PassScheduler {
    ...
    pub fn run(&self, program: Program) -> Result<Program, OptimizerError> {
        ...
        for _ in 0..self.max_iterations {
            ...
            let (next, changed, changed_by) = self.run_once_flags(
                program,
                &dirty,
                &mut next_dirty,
                &mut gates,
        ...
        for &pass_index in &self.execution_order {
            ...
            if dirty.get(pass_index).copied().unwrap_or(false) && pass.analyze(&program).should_run
            {
                if let Some(b) = &mut budget {
                    if let Err(crate::optimizer::pass_result::RefusalReason::BudgetExceeded {
                        resource,
                        budget,
                        consumed,
                    }) = b.charge_cpu_steps(1)
                    {
                        return Err(OptimizerError::BudgetExceeded {
                            resource,
                            budget,
                            consumed,
                        });
                ...
                let (next_program, landed) = if enforce_gates {
                    ...
                    match pass.try_batch_apply(program, &self.adapter) {
                        ...
                        Err(crate::optimizer::pass_result::RefusalReason::BudgetExceeded {
                            resource,
                            budget,
                            consumed,
                        }) => {
                ...
                if landed {
                    if let Some(b) = &mut budget {
                        ...
                        if let Err(crate::optimizer::pass_result::RefusalReason::BudgetExceeded {
                            ...
                            budget,
                            consumed,
                        }) = b.charge_memory_bytes(allocs.bytes as u64)
                        {
                            return Err(OptimizerError::BudgetExceeded {
                                resource,
                                budget,
            ...
            if dirty.get(pass_index).copied().unwrap_or(false) {
                ...
                let (next_program, landed_changed) = if enforce_gates {
                    ...
                    match result {
                        ...
                        Err(refusal) => {
                            if let crate::optimizer::pass_result::RefusalReason::BudgetExceeded {
                                resource,
                                budget,
                                consumed,
                            } = refusal
