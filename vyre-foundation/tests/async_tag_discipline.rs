...
//! Both reference executors already refuse a second start of a tag in flight
//! and a wait with nothing pending, but they refuse it at run time, on the one
//! input that was executed. Nothing said so at compile time, so the defect
//! reached a backend and came back as wrong numbers.
//!
//! The rule decides whether multi-stage pipelining is safe to generate. A
...
fn assert_clean(entry: Vec<Node>, why: &str) {
    ...
    assert!(
        !reported
            .iter()
            .any(|found| found.as_str() == "V131" || found.as_str() == "V132"),
        "{why}: {reported:?}"
    );
}
...
fn a_start_after_the_wait_is_accepted() {
    assert_clean(
        vec![load("stage0"), wait("stage0"), load("stage0")],
...
#[test]
fn a_loop_whose_trip_count_is_not_literal_is_not_reported() {
    assert_clean(
        vec![Node::Loop {
            var: "i".into(),
            from: Expr::u32(0),
