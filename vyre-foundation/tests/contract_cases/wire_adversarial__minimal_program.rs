...
fn assert_decode_fails(
    result: std::thread::Result<Result<Program, vyre_foundation::IrError>>,
    hint: &str,
) {
    match result {
...
fn from_wire_invalid_un_op_tag() {
    let program = Program::wrapped(
        vec![BufferDecl::output("out", 0, DataType::U32).with_count(1)],
        [1, 1, 1],
        vec![Node::store("out", Expr::u32(0), Expr::negate(Expr::u32(1)))],
    );
    let mut bytes = program.to_wire().unwrap();
...
fn opaque_payload_encoder_decoder_cap_symmetry() {
    ...
    assert!(
        error.to_string().contains("opaque node payload")
            && error
                .to_string()
                .contains(&MAX_OPAQUE_PAYLOAD_LEN.to_string()),
