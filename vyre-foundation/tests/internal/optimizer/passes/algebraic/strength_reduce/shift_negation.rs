...
/// A multiply-subtract keeps both roundings.
///
/// `c - a * b` as an FMA rounds once, so the rewrite answers a different
/// function than the program states. Whether the target fuses the pair is the
/// dispatch's float lowering mode to decide, in the emitter that knows it.
#[test]
fn reverse_float_mul_sub_is_not_contracted() {
    let result = reduce_expr(&Expr::sub(
        Expr::f32(1.0),
        Expr::mul(Expr::var("a"), Expr::var("b")),
    ));
    assert!(
        !matches!(result, Some(Expr::Fma { .. })),
        "Fix: strength reduction contracted `c - a * b` into one rounding: {result:?}"
    );
}
