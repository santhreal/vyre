...
pub(crate) fn cast_simple_fixture() -> (Vec<u32>, Vec<u32>) {
    let tok_types = c_kinds("LPAREN INT RPAREN IDENTIFIER SEMICOLON");
    let tok_lens = vec![1; tok_types.len()];
    (tok_types, tok_lens)
}

pub(crate) fn paren_expr_fixture() -> (Vec<u32>, Vec<u32>) {
    let tok_types = c_kinds("LPAREN IDENTIFIER PLUS IDENTIFIER RPAREN SEMICOLON");
    let tok_lens = vec![1; tok_types.len()];
    (tok_types, tok_lens)
}
...
pub(crate) fn cast_complex_fixture() -> (Vec<u32>, Vec<u32>) {
    // (const int *)p;
    let tok_types = c_kinds("LPAREN CONST INT STAR RPAREN IDENTIFIER SEMICOLON");
    let tok_lens = vec![1; tok_types.len()];
    (tok_types, tok_lens)
}
...
pub(crate) fn paren_nested_fixture() -> (Vec<u32>, Vec<u32>) {
    // ((a));
    let tok_types = c_kinds("LPAREN LPAREN IDENTIFIER RPAREN RPAREN SEMICOLON");
    let tok_lens = vec![1; tok_types.len()];
    (tok_types, tok_lens)
}
...
pub(crate) fn typeof_typename_fixture() -> (Vec<u32>, Vec<u32>) {
    let tok_types = c_kinds("GNU_TYPEOF LPAREN INT RPAREN SEMICOLON");
    let tok_lens = vec![1; tok_types.len()];
    (tok_types, tok_lens)
}
