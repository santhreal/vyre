...
fn cuda_executes_retained_iterative_state_across_steps() {
    ...
    // Step 0: s = 0, u = 5 -> s = 5, y = 15
    {
        let mut bindings = session.bindings().expect("bindings");
        ...
        let completion = session.submit_and_wait(bindings).expect("step 0 execution");
        assert_eq!(completion.artifact, session.artifact().unwrap());
        let y_res = session.resource("y").unwrap();
        assert_eq!(
            completion.outputs.get(&y_res).unwrap(),
            &15_u32.to_le_bytes().to_vec()
        );
        let s_res = session.resource("s").unwrap();
        assert_eq!(
            completion.retained.get(&s_res).unwrap(),
            &5_u32.to_le_bytes().to_vec()
        );
    }

    // Step 1: s = 5, u = 3 -> s = 13, y = 23
    {
        let mut bindings = session.bindings().expect("bindings");
        if let Ok(u_val) = session.resource("in_u") {
            bindings.insert(u_val, BoundResource::Host(3_u32.to_le_bytes().to_vec()));
        }
        if let Ok(s_val) = session.resource("s") {
            bindings.insert(s_val, BoundResource::Host(5_u32.to_le_bytes().to_vec()));
        }
        if let Ok(y_val) = session.resource("y") {
            bindings.insert(y_val, BoundResource::Host(vec![0u8; 4]));
        }

        let completion = session.submit_and_wait(bindings).expect("step 1 execution");
        assert_eq!(completion.artifact, session.artifact().unwrap());
        let y_res = session.resource("y").unwrap();
        assert_eq!(
            completion.outputs.get(&y_res).unwrap(),
            &23_u32.to_le_bytes().to_vec()
        );
        let s_res = session.resource("s").unwrap();
        assert_eq!(
            completion.retained.get(&s_res).unwrap(),
