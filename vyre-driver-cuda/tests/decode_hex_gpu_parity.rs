...
fn dispatch_hex(input: &[u8]) -> Vec<u32> {
    ...
    let outputs = backend
        ...
        .expect("Fix: CUDA hex_decode dispatch must succeed");
...
#[test]
fn cuda_hex_decode_matches_reference_for_mixed_case_and_invalid_nibbles() {
    let input = b"0001020a0FfF7gzzA5";
    assert_eq!(dispatch_hex(input), hex_decode_reference_packed(input));
}

#[test]
fn cuda_hex_decode_crosses_workgroup_lanes() {
