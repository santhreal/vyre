...
fn assert_forward_closure_matches(
    ...
    let cpu = reference_forward_closure_via_change_flag(CsrClosureInputs { graph: CsrGraphView { node_count: n, edge_offsets: off, edge_targets: tgt, edge_kind_mask: msk }, allow_mask: allow, max_iters: max_iters }, seed);
    with_cuda_optimizer_dispatcher(label, |dispatcher| {
        let gpu = forward_closure_via_change_flag_gpu(dispatcher, CsrClosureInputs { graph: CsrGraphView { node_count: n, edge_offsets: off, edge_targets: tgt, edge_kind_mask: msk }, allow_mask: allow, max_iters: max_iters }, seed)
        .expect("dispatch");
