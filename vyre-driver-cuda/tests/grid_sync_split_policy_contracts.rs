...
/// The driver must refuse a missing native barrier rather than splitting, on EVERY
/// grid-sync entry point.
///
/// This is a source-level check because the refusal path cannot be reached on
/// hardware that has cooperative launch, and this host does (compute capability
/// 12.0). It is still worth pinning: the silent split lived at two separate call
/// sites, and the previous guard against it grepped `src/lib.rs`, where the call
/// never was. Grepping the file that actually routes is the difference between a
/// test that would have caught this and one that passed throughout.
#[test]
fn no_dispatch_entry_point_reroutes_a_missing_native_barrier_into_a_split() {
    ...
    // The split helper itself stays: it serves the legitimate over-residency route.
    assert!(
...
/// Every internal use of the split helper must be gated on the residency
/// predicate, never on the capability flag alone.
///
/// The failure this locks out is a new call site: someone adding a third dispatch
/// entry point and reaching for `dispatch_borrowed_with_grid_sync_split` directly
/// would silently restore hidden splitting without touching any line these other
/// tests inspect. Counting call sites makes the addition visible.
#[test]
