...
Layer `concrete-backend`. Owner `cuda-driver`.
...
- `vyre-libs` over the `product-libraries` seam, private: composition trees the
  CUDA adapters plan against. Built when: always.
...
- `vyre` over the `cuda-driver` seam, private.
- `vyre-bench` over the `cuda-driver` seam, private.
- `vyre-conform` over the `cuda-driver` seam, private.
- `vyre-registry-link` over the `cuda-driver` seam, private.
...
- Every edge above is declared in `docs/CRATE_OWNERSHIP.toml`. An edge in
  ...
  declares, both fail.
- The crate declares 1 feature beyond `default`: `cuda`. It builds alone.
- The public surface is recorded in `docs/public-api/vyre-driver-cuda.txt`. An
