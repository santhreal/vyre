...
use vyre_driver::{
    dispatch_configs_share_launch_shape, BackendError, DispatchConfig, OutputBuffers,
};
...
use crate::pipeline::{
    cuda_graph_lane_count_for_batch, CudaCompiledPipeline, MaterializedPipelineOutputCache,
    MaterializedPipelineOutputCacheEntry, MAX_GRAPH_CACHE_ENTRIES_PER_PIPELINE,
};
...
#[derive(Debug)]
pub(crate) struct CachedGraphReplaySelection {
    pub(crate) graph: CachedCudaGraph,
    pub(crate) input_state: CudaGraphReplayInputState,
}

impl CudaCompiledPipeline {
    pub(crate) fn dispatch_resident_batches_into(
        &self,
    ...
    pub(crate) fn dispatch_dynamic_persistent_batches_concurrently(
        &self,
        resident_batches: &[SmallVec<[crate::backend::CudaResidentBuffer; 8]>],
        config: &DispatchConfig,
        outputs: &mut Vec<OutputBuffers>,
    ...
    pub(crate) fn dispatch_borrowed_batched_via_cuda_graph_lanes(
        &self,
        batches: &[&[&[u8]]],
        config: &DispatchConfig,
        outputs: &mut Vec<OutputBuffers>,
    ...
    pub(crate) fn materialized_output_cache_hit_with_key_into(
        &self,
        inputs: &[&[u8]],
        input_key: &ExactInputKey,
    ...
    pub(crate) fn remember_materialized_output_cache_with_key(
        &self,
        inputs: &[&[u8]],
        input_key: ExactInputKey,
    ...
    pub(crate) fn take_cached_graph_with_key(
        &self,
        inputs: &[&[u8]],
        input_key: &ExactInputKey,
    ...
    pub(crate) fn take_cached_graph_with_replay_state(
        &self,
        inputs: &[&[u8]],
        input_key: &ExactInputKey,
    ...
    pub(crate) fn return_cached_graph(&self, cached: CachedCudaGraph) -> Result<(), BackendError> {
        let mut graphs = self.graph_cache.lock().map_err(|_| {
            BackendError::DispatchFailed {
                code: None,
    ...
    pub(crate) fn return_cached_graph_lanes(
        &self,
        lanes: SmallVec<[CachedCudaGraph; MAX_GRAPH_CACHE_ENTRIES_PER_PIPELINE]>,
    ) -> Result<(), BackendError> {
        let mut graphs = self.graph_cache.lock().map_err(|_| {
