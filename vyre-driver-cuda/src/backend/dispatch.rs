...
impl CudaBackend {
    ...
    pub fn honors_float_lowering(&self, mode: vyre_foundation::fp_parity::FloatLoweringMode) -> bool {
        match mode {
    ...
    /// hold device storage for a dispatch, so their totals are what an artifact
    /// bound while it was alive. The compiler reconciles this against the
    /// allocation plan it selected, and only for the instance that ran: a
    /// figure read from a materializer before any launch counts whatever the
    /// allocator held for something else.
