...
pub(super) fn check(requirement: &Requirement, base_dir: &Path, failures: &mut Vec<String>) {
        ...
        if !entries.iter().any(|entry| {
            entry.get("name").and_then(serde_json::Value::as_str) == Some("vyre-frontend-c")
                && entry.get("version").and_then(serde_json::Value::as_str)
                    == Some(crate::release::release_train::vyre_frontend_c_version())
            ...
            failures.push(
                format!(
                    ...
                    crate::release::release_train::vyre_frontend_c_version()
                ),
            );
        }
    ...
    for issue in crate::release::package_readiness::package_content_evidence_issues(&package_readiness) {
        failures.push(format!(
