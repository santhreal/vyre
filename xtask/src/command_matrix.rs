...
fn render_markdown(rows: &[CommandMatrixRow]) -> String {
    ...
    // The rendered table carries counts but not their derivation, so the generated doc has
    // to state it: the docs gate requires this page to explain source-count provenance and
    // the source digest, since a bare count with no stated basis is not reviewable.
    out.push_str("Source-count provenance is versioned as `");
    out.push_str(SOURCE_COUNT_PROVENANCE_VERSION);
    out.push_str("`: the count is the primary command file plus its module submodules plus every file the command declares as a shared source. A source digest accompanies each row in the JSON form of this matrix (`source_manifest_digest` over every counted file, and `shared_source_digest` over the shared files alone), so a changed count can be told apart from a changed file.\n\n");
    out.push_str(
...
mod tests {
    ...
    fn shared_frontier_leaderboard_data_is_declared_for_release_benchmarks() {
        assert_eq!(
            ...
            &[
                ARTIFACT_PATHS_SOURCE,
                // `release_benchmarks` shares the benchmark-evidence semantics helpers, so
                // they belong in its declared shared surface.
                BENCHMARK_EVIDENCE_SEMANTICS_SOURCE,
                FRONTIER_LEADERBOARD_BASELINES_DATA_SOURCE,
                HASH_SOURCE,
                RESEARCH_KEY_SOURCE,
