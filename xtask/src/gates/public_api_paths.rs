...
///
/// A name is read from a module-scope `pub` declaration, which is the same text
/// a reader searches for. A name generated by a macro is not seen, so a macro
/// that plants one public name into several modules still reports as a duplicate
/// and the fix is to stop publishing the generated module.
#[must_use]
