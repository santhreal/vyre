...
//!
//! Every workspace member declares one publication class in
//! `[package.metadata.vyre]`, and every publishable member declares, in the same
//! table, the contract that requires it on the registry. This gate reads the
//! committed public-API snapshots from `docs/public-api/` and classifies every
//! exported item by owner, stability, feature and wire compatibility.
//!
...
pub struct PackageClassification {
    ...
    /// Owning seam.
    pub owner: String,
    /// Publication class from the member manifest.
