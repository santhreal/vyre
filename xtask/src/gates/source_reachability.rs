            ...
            let text = match tree.read(&rel) {
                ...
                Err(error) => {
                    report.find(Finding::in_file(
                        &rel,
                        format!(
                            "file is reached from {label} but unreadable: {}",
                            error.message
                        ),
...
impl Gate for IncludeIsNotAModule {
    ...
    fn run(&self, ctx: &GateCtx) -> Result<Report, GateError> {
        let tree = Tree::open(&ctx.root)?;
        let mut report = Report::clean();
        let tracked: BTreeSet<String> = tree.paths().iter().map(|path| as_key(path)).collect();
...
impl Gate for SourceParses {
    ...
    fn run(&self, ctx: &GateCtx) -> Result<Report, GateError> {
        ...
        for (template_root, parsing) in &parsing_templates {
            let owned = exempt
                .values()
                .filter(|owner| *owner == template_root)
                .count();
