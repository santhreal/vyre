...
pub(crate) const TRUTH_FILES: [&str; 4] = [
    "cve_replay.toml",
    "property.toml",
    "differential.toml",
    "e2e_cli.toml",
...
pub(crate) fn truth_dir(slug: &str) -> PathBuf {
...
/// its prefix.
pub(crate) fn escapes_repository(path: &Path) -> bool {
    let root = repo_root();
...
pub(crate) fn require_inside_repository(path: &Path) {
