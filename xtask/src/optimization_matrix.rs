...
const REQUIRED_ANALYSES: &[(&str, &str, &str)] = &[
    ...
    (
        ...
        "External alias/points-to facts",
    ),
    (
        "external-reaching-def",
        "src/analyses/reaching_def_import/mod.rs",
        "External reaching-definition facts",
    ),
];
...
const REQUIRED_INTEGRATION_MARKERS: &[(&str, &str, &str, &str)] = &[
    (
        ...
        "DSE consumes external alias facts",
    ),
    (
        "reaching-def-dse-entrypoint",
        "src/rewrites/dead_store/mod.rs",
        "dead_store_with_dataflow_analysis_facts",
        "DSE consumes external reaching-definition facts",
    ),
    (
        "alias-aware-stlf-entrypoint",
        "src/rewrites/load_forwarding/mod.rs",
        "load_forwarding_with_external_alias_facts",
        "Store/load forwarding consumes external alias facts",
    ),
    (
        "reaching-def-stlf-entrypoint",
        "src/rewrites/load_forwarding/mod.rs",
        "load_forwarding_with_dataflow_analysis_facts",
        "Store/load forwarding consumes external reaching-definition facts",
    ),
    (
        "dataflow-analysis-pipeline-entrypoint",
        "src/rewrites/mod.rs",
        "run_all_with_dataflow_analysis_facts",
        "Canonical rewrite pipeline consumes external alias and reaching-definition facts",
    ),
    (
        "alias-aware-licm-entrypoint",
        "src/rewrites/licm/mod.rs",
        "licm_with_external_alias_facts",
        "LICM consumes external alias facts",
    ),
    (
        "reaching-def-licm-entrypoint",
        "src/rewrites/licm/mod.rs",
        "licm_with_dataflow_analysis_facts",
        "LICM consumes external reaching-definition facts",
    ),
    (
        "alias-aware-loop-fusion-entrypoint",
        "src/rewrites/loop_fusion/mod.rs",
        "loop_fusion_with_external_alias_facts",
        "Loop fusion consumes external alias facts",
    ),
    (
        "reaching-def-loop-fusion-entrypoint",
        "src/rewrites/loop_fusion/mod.rs",
        "loop_fusion_with_dataflow_analysis_facts",
        "Loop fusion consumes external reaching-definition facts",
    ),
    (
        "alias-aware-loop-fission-entrypoint",
        "src/rewrites/loop_fission/mod.rs",
        "loop_fission_with_external_alias_facts",
        "Loop fission consumes external alias facts",
    ),
    (
        "reaching-def-loop-fission-entrypoint",
        "src/rewrites/loop_fission/mod.rs",
        "loop_fission_with_dataflow_analysis_facts",
        "Loop fission consumes external reaching-definition facts",
    ),
    (
        "external-alias-analysis-api",
        "src/analyses/alias_import/mod.rs",
        "AliasFactSet",
        "Vyre lowering imports external alias facts through an explicit API",
    ),
    (
        "external-reaching-def-analysis-api",
        "src/analyses/reaching_def_import/mod.rs",
        "import_descriptor_reaching_defs",
        "Vyre lowering imports external reaching-definition facts through an explicit API",
    ),
    (
        "external-copy-chain-reaching-def-import",
        "src/analyses/reaching_def_import/mod.rs",
        "resolve_copy_alias",
        "External reaching-definition import canonicalizes descriptor Copy chains before rewrite legality checks",
    ),
    (
        ...
        "Integration test proves external reaching-definition facts fire DSE in the canonical pipeline",
    ),
    (
        "dataflow-analysis-stlf-firing-test",
        "src/rewrites/load_forwarding/mod.rs",
        "load_forwarding_with_dataflow_analysis_facts(&desc",
        "Local test proves external reaching-definition facts fire store/load forwarding",
    ),
    (
        ...
        "Integration test proves external facts unlock loop fusion",
    ),
    (
        "dataflow-analysis-loop-fission-firing-test",
        "tests/dataflow_analysis_loop_rewrites.rs",
        "reaching_defs_unlock_alias_proven_loop_fission",
        "Integration test proves external facts unlock loop fission",
    ),
    (
        "dataflow-analysis-licm-firing-test",
        "tests/dataflow_analysis_loop_rewrites.rs",
        "reaching_defs_unlock_alias_proven_licm_load_hoist",
        "Integration test proves external facts unlock LICM load hoisting",
    ),
    (
        "egraph-saturation",
        "src/rewrites/egraph_saturation/mod.rs",
...
fn write_marker_evidence(
    ...
    crate::output_arg::write_json(&path, &evidence);
    Ok(())
}
