...
fn select_retention_set_via_hand_checked_greedy_order_and_early_stop() {
    ...
    let got = select_retention_set_via(&d, &semantic_requests::wrapper_policy(), &mut gains, 4, 3)
        .unwrap();
    assert_eq!(
        got,
        vec![1, 1, 0, 1],
    ...
    let got = select_retention_set_via(&d, &semantic_requests::wrapper_policy(), &mut gains, 3, 1)
        .unwrap();
    assert_eq!(
        got,
        vec![0, 1, 0],
    ...
    let mut gains = vec![0u32, 3, 0];
    let got = select_retention_set_via(&d, &semantic_requests::wrapper_policy(), &mut gains, 3, 3)
        .unwrap();
    ...
    let got = select_retention_set_via(&d, &semantic_requests::wrapper_policy(), &mut gains, 2, 0)
        .unwrap();
