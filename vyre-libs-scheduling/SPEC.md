...
Layer `libraries`. Owner `libs-scheduling`.

## Owns

Compiler-internal scheduling compositions: fusion candidates, batching, and dispatch strategy expressed as IR and as facts about it.
...
5. A legality decision priced as a cost. Illegality is rejected before ranking, never folded into a number.
6. A persistence choice. Persistence is a selected schedule over one artifact class, decided by `vyre-megakernel`.
...
`vyre-foundation`, `vyre-primitives` and `vyre-spec` must never depend on `vyre-libs-scheduling`. The edge is one way: a cycle back into this crate makes the two crates one crate that cannot be built, reviewed or published apart.
...
- The public surface is recorded in `docs/public-api/vyre-libs-scheduling.txt`. An item added, removed or moved changes that file in the same change.
