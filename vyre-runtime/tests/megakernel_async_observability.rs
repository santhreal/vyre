...
use vyre_runtime::resident_work_queue::{
    ...
    ResidentWorkQueue, ResidentExecutionMode, ResidentLaunchRequest, RingTelemetry,
};
...
fn telemetry_decode_counts_mixed_inflight_states() {
    ...
    ResidentWorkQueue::publish_slot(&mut ring, 2, 3, protocol::opcode::ATOMIC_ADD, &[3, 4]).unwrap();
    ResidentWorkQueue::publish_slot(&mut ring, 3, 4, protocol::opcode::LOAD_U32, &[5, 6]).unwrap();
    ResidentWorkQueue::publish_slot(&mut ring, 4, 5, protocol::opcode::COMPARE_SWAP, &[7, 8, 9]).unwrap();
    ResidentWorkQueue::publish_slot(&mut ring, 5, 6, protocol::opcode::MEMCPY, &[10, 11, 12]).unwrap();
...
fn active_slots_for_opcode_filters_only_inflight() {
    ...
    let telemetry = RingTelemetry::decode(&ResidentWorkQueue::encode_control(false, 1, 0).unwrap(), &ring);
