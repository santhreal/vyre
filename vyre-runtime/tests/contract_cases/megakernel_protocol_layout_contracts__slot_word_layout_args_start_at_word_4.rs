...
fn slot_word_layout_args_start_at_word_4() {
    ...
    ResidentWorkQueue::publish_slot(&mut ring, 0, 0, opcode::NOP, &[0xDEAD_BEEF]).unwrap();
...
fn read_done_count_from_exact_buffer_succeeds() {
    ...
    assert_eq!(ResidentWorkQueue::read_done_count(&ctrl), 12345);
}
...
fn read_epoch_from_exact_buffer_succeeds() {
    ...
    assert_eq!(ResidentWorkQueue::read_epoch(&ctrl), 0x1234_5678);
}
...
fn packed_slot_status_is_published_after_success() {
    ...
    ResidentWorkQueue::publish_packed_slot(&mut ring, 1, 0, &[(opcode::NOP as u8, vec![])])
