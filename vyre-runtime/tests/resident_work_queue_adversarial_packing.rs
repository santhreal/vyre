...
fn batch_publish_with_fence_consumes_expected_slot_count() {
    ...
    let consumed = ResidentWorkQueue::batch_publish(&mut ring, 0, 0, &items, 0xABCD).unwrap();
...
fn batch_publish_accepts_borrowed_arg_slices() {
    ...
    let consumed = ResidentWorkQueue::batch_publish(&mut ring, 0, 0, &items, 0x55AA).unwrap();
...
fn duplicate_batch_publish_to_adjacent_slots_succeeds() {
    ...
    let c1 = ResidentWorkQueue::batch_publish(&mut ring, 0, 0, &items, 1).unwrap();
    let c2 = ResidentWorkQueue::batch_publish(&mut ring, c1, 0, &items, 2).unwrap();
    assert_eq!(c1, 2);
    assert_eq!(c2, 2);
}
...
fn packed_slot_can_be_recycled_after_marking_done() {
    ...
    ResidentWorkQueue::publish_packed_slot(&mut ring, 0, 0, &ops).unwrap();
    write_word(&mut ring, protocol::STATUS_WORD as usize, slot::DONE);

    let ops2 = vec![(protocol::opcode::STORE_U32 as u8, vec![1, 2])];
    ResidentWorkQueue::publish_packed_slot(&mut ring, 0, 0, &ops2)
