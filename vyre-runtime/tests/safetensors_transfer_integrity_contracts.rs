...
use vyre_runtime::safetensors_transfer::{
    select_transfer_path, DeviceTransferCapabilities, SafetensorTransferPath,
    TransferDescriptor, TransferError, TransferLifecycleEngine,
};
...
fn path_selection_proves_unsupported_direct_transfer_never_masquerades_as_zero_copy() {
    ...
    assert_eq!(decision_direct.selected_path, SafetensorTransferPath::DirectStorage);
    ...
    assert_eq!(decision_unaligned.selected_path, SafetensorTransferPath::RegisteredHostMemory);
    ...
    assert_eq!(decision_no_fs.selected_path, SafetensorTransferPath::RegisteredHostMemory);
    ...
    assert_eq!(decision_no_direct.selected_path, SafetensorTransferPath::RegisteredHostMemory);
...
fn transfer_lifecycle_blocks_dispatch_visibility_until_completion() {
    ...
    engine.initiate_transfer(descriptor).expect("Fix: transfer initiation must succeed");
    assert!(!engine.is_dispatch_visible("model.layers.0.weight"));

    // 2. In-progress transfer is NOT dispatch visible
    engine.progress_transfer("model.layers.0.weight", 32768).unwrap();
    assert!(!engine.is_dispatch_visible("model.layers.0.weight"));

    // 3. Completing transfer with wrong digest fails and remains invisible
    let wrong_digest = [99_u8; 32];
    let err = engine.complete_transfer("model.layers.0.weight", 101, wrong_digest)
    ...
    engine.complete_transfer("model.layers.1.weight", 102, [3_u8; 32]).unwrap();
...
fn device_loss_and_cancellation_safely_cleanup_residency() {
    ...
    engine.complete_transfer("model.embed.weight", 200, [4_u8; 32]).unwrap();
...
fn queue_backpressure_enforces_maximum_in_flight_depth() {
    ...
    let err = engine.initiate_transfer(overflow_desc)
