...
fn try_read_observable_rejects_buffer_ending_exactly_at_observable_base() {
    ...
    let err = ResidentWorkQueue::try_read_observable(&buf, 0)
...
fn try_read_observable_accepts_boundary_when_buffer_has_one_extra_word() {
    ...
    let val = ResidentWorkQueue::try_read_observable(&buf, 0)
...
fn try_encode_empty_debug_log_zero_capacity_roundtrips() {
    ...
    let records = ResidentWorkQueue::try_read_debug_log(&log).expect("zero cursor must decode");
...
fn read_metrics_on_empty_buffer_returns_empty() {
    let metrics = ResidentWorkQueue::read_metrics(&[]);
...
fn read_observable_returns_zero_for_out_of_bounds_on_minimal_buffer() {
    ...
    assert_eq!(ResidentWorkQueue::read_observable(&ctrl, 99), 0);
}
...
fn strict_metrics_window_exact_boundary_succeeds() {
    ...
    let metrics = ResidentWorkQueue::try_read_metrics(&buf).expect("exact boundary must succeed");
