...
#[test]
fn typed_ingestion_includes_global_and_constant_target_resources() {
    ...
    for (id, res) in resources.iter().enumerate() {
        dataset.add_resident(ArtifactValueId(id as u32), res.clone());
    }
    ...
    let mut partial_dataset = TypedResourceDataset::new();
    partial_dataset.add_resident(ArtifactValueId(0), resources[0].clone());
    partial_dataset.add_resident(ArtifactValueId(1), resources[1].clone());
...
/// WHY: typed ingestion maps by authenticated canonical value identity rather than
/// vector index, so target slots emitted in non-canonical order bind their exact resource.
#[test]
fn typed_ingestion_preserves_canonical_value_identity_with_reordered_target_slots() {
    let neutral = resident_projection_artifact();
...
/// WHY: typed ingestion fails closed on unknown resources and conflicting duplicates.
#[test]
fn typed_ingestion_fails_closed_on_duplicate_mismatch_and_unknown_values() {
    let neutral = resident_projection_artifact();
    let payload = resident_projection_payload(&neutral);
    let session =
        ArtifactSession::from_bytes(&TEST_REGISTRATION, &envelope_bytes(neutral, [payload]))
    ...
    let mut valid_dataset = TypedResourceDataset::new();
    valid_dataset.add_resident(ArtifactValueId(0), res_a.clone());
    valid_dataset.add_resident(ArtifactValueId(1), res_b.clone());
    valid_dataset.add_resident(ArtifactValueId(2), res_c.clone());
    assert!(session.ingest(&valid_dataset).is_ok());

    let mut unknown_dataset = TypedResourceDataset::new();
    unknown_dataset.add_resident(ArtifactValueId(99), res_a.clone());
    unknown_dataset.add_resident(ArtifactValueId(0), res_a.clone());
    unknown_dataset.add_resident(ArtifactValueId(1), res_b.clone());
    unknown_dataset.add_resident(ArtifactValueId(2), res_c.clone());
    let err = session
        .ingest(&unknown_dataset)
        .expect_err("unknown value must fail closed");
    assert!(err.to_string().contains("99"));

    let mut duplicate_dataset = TypedResourceDataset::new();
    duplicate_dataset
        .insert(TypedResource::resident(ArtifactValueId(0), res_a.clone()))
        .expect("first insert");
    let dup_err = duplicate_dataset
        .insert(TypedResource::resident(ArtifactValueId(0), res_b))
        .expect_err("conflicting duplicate value must fail closed");
    assert!(dup_err.to_string().contains("duplicate conflicting"));
}
