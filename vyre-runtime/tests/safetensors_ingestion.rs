...
fn sharded_index_resolves_every_weight_once() {
    ...
    let index = vyre_runtime::checkpoint::ShardedSafetensorIndex::open(temp.path(), &index_path)
...
fn missing_mapped_tensor_fails_closed() {
    ...
    .expect("Fix: shard index must be writable");
    assert_eq!(
        vyre_runtime::checkpoint::ShardedSafetensorIndex::open(temp.path(), &index_path)
