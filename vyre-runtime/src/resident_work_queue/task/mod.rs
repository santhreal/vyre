...
// Inline: the suite drives the `#[cfg(test)]` panicking accessors (`paused`,
// `resumed`, `yielded`, `requeued`, `completed`, `faulted`, `schedulable_count`,
// `continuation_pressure_count`, `apply_to_launch_request`), which are compiled
// only under test and so are invisible to an integration test.
#[cfg(test)]
mod tests {
    use super::*;
