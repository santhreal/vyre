...
pub fn min_cuda_release_memory_mib() -> u64 {
    let largest_workload_bytes = vyre_bench::registry::collect_all()
        .iter()
...
///
/// The one owner of the comparison. Both the live `nvidia-smi` probe and the
/// recorded-JSON release checks route here so a device cannot qualify for one
...
mod tests {
    ...
    fn the_floor_covers_every_registered_workload() {
        ...
        for case in vyre_bench::registry::collect_all().iter() {
    ...
    fn the_floor_leaves_room_for_the_cuda_context() {
        let largest = vyre_bench::registry::collect_all()
            .iter()
            .filter_map(|case| case.requirements().min_vram_bytes)
            .max()
            .unwrap_or(0);
    ...
    /// WHY: both floors are inclusive, and both are compared by two callers
    /// that used to hold their own copy of the comparison. An off-by-one at
    /// either boundary either rejects a device that runs every workload or
    /// admits one that runs none, and neither shows up as a test failure
    /// anywhere else: the live probe path needs real hardware to reach.
    #[test]
