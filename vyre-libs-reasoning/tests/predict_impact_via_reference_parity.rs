...
fn predict_impact_via_matches_reference_over_random_dags() {
    ...
    for case in 0..300u32 {
        ...
        let got = predict_impact_via(&d, &semantic_requests::wrapper_policy(), &adj, &mask, n_u)
...
fn predict_impact_via_hand_checked_chain() {
    ...
    adj[2 * 4 + 3] = 1;
    let mask = [1u32, 0, 0, 0];
    let got = predict_impact_via(&d, &semantic_requests::wrapper_policy(), &adj, &mask, n).unwrap();
