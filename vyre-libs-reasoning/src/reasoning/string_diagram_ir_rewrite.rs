...
use vyre_libs_graph::graph::string_diagram::monoidal_compose;
use vyre_megakernel::{
...
mod tests {
    ...
    impl SemanticExecutor for ComposeDispatcher {
        fn execute(
            ...
            let f = vyre_libs_builder::plumbing::host::dispatch_buffers::read_u32s(&inputs[0]);
            let g = vyre_libs_builder::plumbing::host::dispatch_buffers::read_u32s(&inputs[1]);
            assert_eq!(f.len(), 4);
            assert_eq!(g.len(), 4);
