...
mod blur {
    ...
    fn program_radius_zero_has_single_weight() {
        ...
        let weights = vyre_libs_math::math::conv1d::gaussian_weights(0, 1.0);
    ...
    fn gaussian_weights_sum_to_one() {
        for radius in [1, 2, 4, 8, 16] {
            ...
            let weights = vyre_libs_math::math::conv1d::gaussian_weights(radius, sigma);
    ...
    fn gaussian_weights_are_symmetric() {
        let weights = vyre_libs_math::math::conv1d::gaussian_weights(4, 1.5);
        let n = weights.len();
        for i in 0..n / 2 {
            assert_eq!(
                weights[i],
...
mod shadow {
    use vyre_libs_visual::visual::box_shadow;
...
mod filter_chain_tests {
    use vyre_libs_visual::visual::filter_chain;
...
mod composite {
    use vyre_libs_visual::visual::alpha_over;
...
mod gradient {
    ...
    fn single_stop_returns_error_contract() {
        let error = vyre_libs_visual::visual::try_linear_gradient(
            "out",
            4,
            4,
            0.0,
...
mod downsample {
    use vyre_libs_visual::visual::downsample_2x;
