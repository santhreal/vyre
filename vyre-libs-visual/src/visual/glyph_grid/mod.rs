...
fn channel(pixel: &str, shift: u32) -> Expr {
    vyre_libs_builder::builder::stencil::unpack_channel(pixel, shift)
}
...
fn blend_channel(shift: u32) -> Expr {
    vyre_libs_builder::builder::stencil::blend_channel_coverage(
        channel("fg_px", shift),
        channel("bg_px", shift),
        Expr::var("cov"),
        Expr::var("inv_cov"),
...
pub fn glyph_grid_blend(
    ...
    body.push(Node::let_bind(
        "packed",
        vyre_libs_builder::builder::stencil::pack_rgba_named("out_r", "out_g", "out_b", "out_a"),
    ));
