...
//!
//! Composes `vyre_libs_math::math::conv1d` for horizontal + vertical
...
impl GaussianKernel {
    ...
    pub fn new(radius: u32, sigma: f32) -> Self {
        let clamped = radius.min(vyre_libs_math::math::conv1d::MAX_RADIUS);
        Self {
            radius: clamped,
            weights: vyre_libs_math::math::conv1d::gaussian_weights(clamped, sigma),
        }
    }
...
fn gaussian_blur_pass(
    ...
    let blur_pass = wrap_child_region(
        vyre_libs_math::math::conv1d::OP_ID,
        parent,
        vec![
            ...
            Node::if_then(Expr::lt(Expr::var("idx"), Expr::u32(count)), {
                ...
                for k in 0..diameter {
                    ...
                    let pixel_idx = vyre_libs_builder::builder::stencil::separable_sample_index(
                        is_horiz,
                        &Expr::var("py"),
                        &Expr::var("px"),
                        offset,
                    ...
                    body.push(Node::assign(
                        ...
                        Expr::add(
                            Expr::var("acc_r"),
                            Expr::mul(
                                vyre_libs_builder::builder::stencil::unpack_channel(&tap_name, 0),
                                Expr::u32(w_val),
                            ),
                        ),
                    ));
                    body.push(Node::assign(
                        ...
                        Expr::add(
                            Expr::var("acc_g"),
                            Expr::mul(
                                vyre_libs_builder::builder::stencil::unpack_channel(&tap_name, 8),
                                Expr::u32(w_val),
                            ),
                        ),
                    ));
                    body.push(Node::assign(
                        ...
                        Expr::add(
                            Expr::var("acc_b"),
                            Expr::mul(
                                vyre_libs_builder::builder::stencil::unpack_channel(&tap_name, 16),
                                Expr::u32(w_val),
                            ),
                        ),
                    ));
                    body.push(Node::assign(
                        ...
                        Expr::add(
                            Expr::var("acc_a"),
                            Expr::mul(
                                vyre_libs_builder::builder::stencil::unpack_channel(&tap_name, 24),
                                Expr::u32(w_val),
                            ),
                        ),
                    ));
                ...
                body.push(Node::let_bind(
                    "packed",
                    vyre_libs_builder::builder::stencil::pack_rgba(
                        Expr::var("or"),
                        Expr::var("og"),
                        Expr::var("ob"),
                        Expr::var("oa"),
