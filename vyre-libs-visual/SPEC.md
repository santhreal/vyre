...
Layer `libraries`. Owner `libs-visual`.

## Owns

Visual compositions: blur, shadow, filters, gradients, and glass compositing effects.
...
5. A windowing system, a surface handle, or a swapchain. Presentation is an application concern.
6. A color management decision taken from a display profile at build time.
...
`vyre-foundation`, `vyre-primitives` and `vyre-spec` must never depend on `vyre-libs-visual`. The edge is one way: a cycle back into this crate makes the two crates one crate that cannot be built, reviewed or published apart.
...
- The public surface is recorded in `docs/public-api/vyre-libs-visual.txt`. An item added, removed or moved changes that file in the same change.
