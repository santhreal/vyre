    ...
    for node in graph.nodes() {
        ...
        if snapshot_is_live {
            ...
            let unmet = [
                ...
                (
                    required.subgroup_ops && !capabilities.has_subgroup_shuffle,
                    "program uses subgroup operations but the device reports no subgroup shuffle",
                    "remove the subgroup operation, or compile for a device with subgroup-level shuffle",
                ),
                (
