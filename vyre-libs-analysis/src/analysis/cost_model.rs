...
/// # Errors
///
/// Returns [`SemanticExecutionError`] for malformed circuit buffers, unsorted residuals,
/// invalid alpha, rejected semantic execution, or malformed backend output.
...
pub fn predict_runtime_fixed_via_with_scratch(
    ...
    let circuit_outputs = execute_single_program(
        dispatcher,
        vyre_libs_builder::plumbing::host::dispatch_buffers::HOST_WRAPPER_NODE,
        circuit,
        &scratch.inputs[..8],
        policy,
    )
    ...
    let conformal_outputs = execute_single_program(
        dispatcher,
        vyre_libs_builder::plumbing::host::dispatch_buffers::HOST_WRAPPER_NODE,
        conformal,
        &scratch.inputs[..2],
...
fn validate_circuit(
    ...
    if children.is_empty() {
        return Err(SemanticExecutionError::InvalidRequest("Fix: predict_runtime_fixed_via requires at least one circuit edge for the dispatchable sum-product primitive.".to_string()));
    }
...
fn validate_sorted_residuals(residuals: &[u32]) -> Result<(), SemanticExecutionError> {
...
mod tests {
    ...
    use vyre_libs_graph::graph::sum_product_circuit::{KIND_LEAF, KIND_PRODUCT, KIND_SUM};
    ...
    impl SemanticExecutor for CostModelDispatcher {
        fn execute(
            &self,
    ...
    fn dispatch_sum_product(inputs: &[Vec<u8>]) -> Result<Vec<Vec<u8>>, SemanticExecutionError> {
        let depths = vyre_libs_builder::plumbing::host::dispatch_buffers::read_u32s(&inputs[0]);
        let kinds = vyre_libs_builder::plumbing::host::dispatch_buffers::read_u32s(&inputs[1]);
        let offsets = vyre_libs_builder::plumbing::host::dispatch_buffers::read_u32s(&inputs[2]);
        let counts = vyre_libs_builder::plumbing::host::dispatch_buffers::read_u32s(&inputs[3]);
        let children = vyre_libs_builder::plumbing::host::dispatch_buffers::read_u32s(&inputs[4]);
        let weights = vyre_libs_builder::plumbing::host::dispatch_buffers::read_u32s(&inputs[5]);
        let values = vyre_libs_builder::plumbing::host::dispatch_buffers::read_u32s(&inputs[6]);
    ...
    fn dispatch_conformal(inputs: &[Vec<u8>]) -> Result<Vec<Vec<u8>>, SemanticExecutionError> {
        let scores = vyre_libs_builder::plumbing::host::dispatch_buffers::read_u32s(&inputs[0]);
