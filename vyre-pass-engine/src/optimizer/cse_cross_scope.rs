//! Cross-scope expression CSE hoisting over the returned Program.
//!
//! Handles the same canonical Expr appearing directly as the value of
//! `Node::Store` / `Node::Assign` / `Node::If` cond / `Node::Loop` bounds:
//! when it occurs at two or more such top-level positions in a scope, a fresh
//! `let __cse_<n> = E;` is introduced at the scope start and every occurrence
//! becomes `Var(__cse_<n>)`.
//!
//! The walker keys off the arena's `node_top_level_exprs`, so an upstream
//! rewrite that changes inner Expr shape does not disturb it: Node-level
//! structure is what it indexes.

use std::sync::Arc;

use rustc_hash::FxHashMap;
use vyre_foundation::ir::{Expr, Ident, Node, Program};
use vyre_foundation::optimizer::passes::expr_is_atomic_free;
use vyre_foundation::transform::rewrite_walk::{
    extend_with_rewritten_scope, reachable_prefix, rewrite_program_entry, visit_scope, NodeRewrite,
};
...
pub fn apply_cross_scope_cse_with_lookup<C: CanonicalLookup + ?Sized>(
    ...
    rewrite_program_entry(program, |body| hoister.rewrite_scope(body))
}
...
impl<C: CanonicalLookup + ?Sized> Hoister<'_, C> {
    ...
    fn rewrite_scope(&mut self, body: &[Node]) -> Vec<Node> {
        let reachable_len = reachable_prefix(body).len();
        ...
        for canon in &order {
            ...
            if count < 2 || !is_hoist_worthy(&expr) || !expr_is_atomic_free(&expr) {
                continue;
            }
        ...
        extend_with_rewritten_scope(body, &mut substitute, &mut out);
        out
    }
