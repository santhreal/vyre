...
fn parity_matrix_input_planner_tracks_dynamic_fixture_contract() {
    let path = "conform/vyre-conform/src/witness_plan.rs";
    ...
    let main = repo_file("conform/vyre-conform/src/main.rs");
    assert!(
    ...
    let parity = repo_file("conform/vyre-conform/tests/__split/parity_matrix_chunk1.rs");
    assert!(
...
fn bundle_certificate_uses_shared_witness_input_planner() {
    let source = repo_file("conform/vyre-conform/src/bundle_cert.rs");
...
fn ulp_audit_input_planner_tracks_release_witness_contract() {
    let source = repo_file("conform/vyre-conform/tests/ulp_audit.rs");
    let plan = repo_file("conform/vyre-conform/tests/__split/ulp_audit_input_plan.rs");
    let split = repo_file("conform/vyre-conform/tests/__split/ulp_audit_part1.rs");
