...
Commands: `abstraction-gate`, `application-runnable`, `catalog`, `compile`, `configuration-model`, `conformance-matrix`, `cross-target`, `dialect-lowering`, `error-codes`, `gate1`, `heuristic-audit`, `lego-composability`, `lego-composition-chains`, `lego-composition-depth`, `lego-cross-dialect`, `lego-duplicate-report`, `lego-exemption-liveness`, `lego-name-stems`, `lego-no-reinvention`, `lego-operand-shapes`, `lego-primitive-coverage`, `lego-semantic-organization`, `lego-tier-claims`, `lego-trend`, `list-ops`, `op-matrix`, `operation-law-decisions`, `operation-schema`, `optimization-corpus`, `optimization-docs`, `optimization-matrix`, `print-composition`, `shrink`, `trace-f32`, `verify-rewrite-proofs`, `whats-similar`.
...
```console
./cargo_full run -p xtask-registry -- --help
```
