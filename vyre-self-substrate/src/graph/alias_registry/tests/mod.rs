...
fn alias_registry_uses_dedicated_observability_counter() {
    let alias_before = vyre_libs::telemetry::observability::alias_registry_calls.load(Ordering::Relaxed);
    let dataflow_before = vyre_libs::telemetry::observability::dataflow_fixpoint_calls.load(Ordering::Relaxed);

    let registry = build_default_registry();
    assert!(alias_union_registered(&registry));
    assert!(lookup_alias_op(&registry, ALIAS_UNION_OP_ID).is_some());

    let alias_after = vyre_libs::telemetry::observability::alias_registry_calls.load(Ordering::Relaxed);
    let dataflow_after = vyre_libs::telemetry::observability::dataflow_fixpoint_calls.load(Ordering::Relaxed);
    assert!(
