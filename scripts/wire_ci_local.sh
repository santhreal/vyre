...
./cargo_full fmt -p vyre-primitives -- --check vyre-primitives/src/wire.rs

log "clippy, wire crates (--no-deps keeps the gate scoped to our code)"
./cargo_full clippy -p vyre-primitives --no-deps \
    --features "matching cpu-parity hash inventory-registry" -- -D warnings
./cargo_full clippy -p vyre-libs --no-deps -- -D warnings

log "check, wire and consumers"
./cargo_full check -p vyre-primitives
./cargo_full check -p vyre-libs
./cargo_full check -p vyre-pass-engine
./cargo_full check -p vyre-bench
./cargo_full check -p vyre-driver

log "test, wire contracts (positive + negative + property + differential)"
./cargo_full test -p vyre-primitives --test wire_pack_into_contracts --features matching
./cargo_full test -p vyre-primitives --test wire_differential_std_io --features matching
./cargo_full test -p vyre-primitives --test proptest_wire_roundtrip --features matching

log "test, cross-crate compat"
./cargo_full test -p vyre-libs --test wire_cross_crate_compat

log "harness, build + run the agent-harness smoke binary"
./cargo_full build --release --example wire_harness_smoke -p vyre-primitives
./cargo_full test -p vyre-primitives --test wire_harness_smoke_test --features matching

log "doc-build, wire module doctests"
./cargo_full test --doc -p vyre-primitives wire
...
run_contract_suite() {
    local log="$1"
    if ! ./cargo_full test -p vyre-primitives --test wire_pack_into_contracts --features matching \
