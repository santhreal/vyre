#!/usr/bin/env python3
"""Assert both halves of the internal-dependency version rule for publishable members.

Usage: check_internal_deps_have_versions.py ROOT
...
def judge(rel: str, table: str, key: str, spec: object, lines: dict) -> None:
    ...
    if not members[package][1]:
        if versioned:
            problems.append(
                ...
                f"requirement on an unpublishable crate, so `./cargo_full package` "
                f"fails here."
            )
        return
    if not versioned and table != "dev-dependencies" and not table.endswith(".dev-dependencies"):
        problems.append(
            ...
            f"against a versioned table entry. Path-only blocks `./cargo_full publish` "
            f"from resolving siblings on the registry."
        )
