...
mod tests {
    ...
    /// Test CPU reference for the subgroup shuffle intrinsic.
    ///
    /// # Panics
    ...
    fn test_cpu_ref(values: &[u32], lanes: &[u32]) -> Vec<u8> {
    ...
    fn cpu_ref_oob_lane_panics_with_actionable_message() {
        ...
        let result = std::panic::catch_unwind(|| test_cpu_ref(&values, &lanes));
        let err = result.expect_err("expected test_cpu_ref to panic on OOB lane index");
        let msg = err
            .downcast_ref::<String>()
            .map(|s| s.as_str())
    ...
    fn registration_fixture_matches_exact_byte_constant() {
        assert_eq!(
            EXPECTED_SUBGROUP_SHUFFLE_OUTPUT_BYTES.to_vec(),
            test_cpu_ref(&[10, 20, 30, 40], &[0, 1, 0, 2])
        );
    }
}
