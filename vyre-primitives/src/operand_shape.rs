...
//! neither may enable the other. `math` enables `graph`, so the reverse edge
//! would be a feature cycle; while these sat in `math`, the seven graph call
//! sites and the fixed-point matmul builder only compiled because some other
//! workspace member happened to enable `math` too. A consumer enabling `graph`
//! alone against the published crate failed with E0433.
