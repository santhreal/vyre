...
inventory::submit! {
    vyre_foundation::operation::OperationRegistration::primitive(
        ...
        Some(|| {
            vec![vec![
                crate::wire::pack_u32_slice(&[0, 0, 1, IDOM_NONE]),
                crate::wire::pack_u32_slice(&[0; 4]),
            ]]
        }),
        Some(|| vec![vec![crate::wire::pack_u32_slice(&[0, 1, 2, 0])]]),
    )
}
...
inventory::submit! {
    vyre_foundation::operation::OperationRegistration::primitive(
        ...
        Some(|| {
            vec![vec![
                crate::wire::pack_u32_slice(&[0, 0, 1, 2, 4]),
                crate::wire::pack_u32_slice(&[0, 0, 1, 2]),
                crate::wire::pack_u32_slice(&[0, 0, 0, IDOM_NONE]),
                crate::wire::pack_u32_slice(&[0, 1, 1, 0]),
