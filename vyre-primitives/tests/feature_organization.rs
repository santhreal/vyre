...
//!
//! An operation is admitted to this crate only when it cannot be composed,
//! which means it needs its own arm in a backend emitter and its own arm in the
//! reference interpreter. Marker types are always on; `hardware` is the
//! intrinsic domain. Every composition domain that was parked in the crate has
//! moved to `vyre-libs`, so the composition list is empty and must stay empty.
//!
//! The lists live here rather than in the crate, because nothing the crate
//! ships reads them: they exist to be held against the manifest. A feature
//! added to `Cargo.toml` without an entry in one of them fails
//! `every_cargo_feature_is_classified_exactly_once`.
...
const INTRINSIC_FEATURES: &[&str] = &["hardware"];
...
const COMPOSITION_FEATURES: &[&str] = &[];

/// Crate-support features. Not domains.
const SUPPORT_FEATURES: &[&str] = &["cpu-parity", "gpu", "inventory-registry", "vyre-foundation"];
...
fn every_cargo_feature_is_classified_exactly_once() {
    ...
    assert_eq!(
        declared,
        classified,
        "Fix: Cargo.toml [features] and tests/feature_organization.rs disagree.\n\
         only in Cargo.toml: {:?}\n\
         only in feature_organization.rs: {:?}",
...
fn every_classified_domain_gates_a_module_in_this_crate() {
    let src = vyre_test_support::monorepo::vyre_crate_directory(env!("CARGO_PKG_NAME")).join("src");
