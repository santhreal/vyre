...
[`tests/feature_organization.rs`](tests/feature_organization.rs) is the one
classification of every Cargo feature, and its
`every_cargo_feature_is_classified_exactly_once` test fails if a new
feature is added without being listed there. The lists live in the test
because nothing the crate ships reads them: they exist to be held
