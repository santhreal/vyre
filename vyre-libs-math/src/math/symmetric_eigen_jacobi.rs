...
//! reduction (`vyre_libs_builder::builder::cooperative::Argmax`), the identity seeding, the sign pass and the
...
pub fn symmetric_eigen_jacobi(a: &str, eigenvectors: &str, eigenvalues: &str, n: u32) -> Program {
    let cells = match vyre_libs_builder::plumbing::operand::shape::square_matrix_cells(OP_ID, n) {
        Ok(cells) => cells,
