...
use vyre_libs_builder::builder::gemm::{ContractionComposer, ContractionGeometry};
...
inventory::submit! {
    vyre_foundation::operation::OperationRegistration::library_unconstrained(
        ...
        Some(|| {
            // A = [[1, 2], [3, 4]], B = [[5, 6], [7, 8]]
            let a = vyre_libs_builder::fixture_bytes::f32_bytes(&[1.0, 2.0, 3.0, 4.0]);
            let b = vyre_libs_builder::fixture_bytes::f32_bytes(&[5.0, 6.0, 7.0, 8.0]);
            vec![vec![a, b]]
        }),
        Some(|| {
            // C = A · B = [[19, 22], [43, 50]]
