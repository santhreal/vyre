...
use crate::signed_coverage::SignedCoverage;
use vyre_libs_math::math::randomized_svd::randomized_projection_step;
...
fn randomized_projection_signed_matches_exact_fixed_point_matmul() {
    ...
    let mut coverage = SignedCoverage::default();
    for case in 0..400u32 {
        ...
        coverage.operands(a.iter().chain(&omega));
        ...
        coverage.result(&want);
    }
    coverage.assert_floors("randomized_svd::randomized_projection_step", 500, 100, 380);
}
