...
fn lattice_join_commutative() {
    ...
    let p1 = vyre_libs_math::math::algebra::lattice_join("a", "b", "out", 2);
    let p2 = vyre_libs_math::math::algebra::lattice_join("b", "a", "out", 2);

    let o1 = eval_pair(&p1, &a, &b);
    let o2 = eval_pair(&p2, &b, &a);

    assert_eq!(o1.clone(), o2.clone(), "join must be commutative");
}
...
fn lattice_join_associative() {
    ...
    // (x | y) | z
    let p_xy = vyre_libs_math::math::algebra::lattice_join("x", "y", "xy", 1);
...
fn lattice_join_identity_all_zeros() {
    ...
    let zero = [0u32, 0];
    let program = vyre_libs_math::math::algebra::lattice_join("a", "zero", "out", 2);
    let outputs = eval_pair(&program, &a, &zero);

    assert_eq!(decode_u32_words(&outputs), vec![0xDEAD_BEEF, 0xCAFE_BABE]);
}
...
fn lattice_join_size_one() {
    let a = [0b1010u32];
    let b = [0b0101u32];
    let program = vyre_libs_math::math::algebra::lattice_join("a", "b", "out", 1);
    let outputs = eval_pair(&program, &a, &b);

    assert_eq!(decode_u32_words(&outputs), vec![0b1111]);
}
...
fn lattice_join_all_ones_absorbs() {
    ...
    let program = vyre_libs_math::math::algebra::lattice_join("a", "ones", "out", 1);
    let outputs = eval_pair(&program, &a, &ones);

    assert_eq!(decode_u32_words(&outputs), vec![0xFFFF_FFFF]);
}
...
fn lattice_join_large_size_power_of_two() {
    ...
    let program = vyre_libs_math::math::algebra::lattice_join("a", "b", "out", n);
    let outputs = eval_pair(&program, &a, &b);

    let expected = vec![0xFFFF_FFFFu32; n as usize];
    assert_eq!(decode_u32_words(&outputs), expected);
}

#[test]
fn try_lattice_join_rejects_aliased_names() {
    let err = vyre_libs_math::math::algebra::try_lattice_join("a", "a", "out", 4).unwrap_err();
    assert!(
...
fn lattice_meet_specific_values() {
    let a = [0x0000_FFFFu32, 0xAAAA_AAAA, 0x0000_0000, 0xFFFF_FFFF];
    let b = [0xFFFF_0000u32, 0x5555_5555, 0x0000_0000, 0x0000_0000];
    let program = vyre_libs_math::math::algebra::lattice_meet("a", "b", "out", 4);
    let outputs = eval_pair(&program, &a, &b);

    assert_eq!(
        decode_u32_words(&outputs),
...
fn lattice_meet_commutative() {
    ...
    let p1 = vyre_libs_math::math::algebra::lattice_meet("a", "b", "out", 1);
    let p2 = vyre_libs_math::math::algebra::lattice_meet("b", "a", "out", 1);

    let o1 = eval_pair(&p1, &a, &b);
    let o2 = eval_pair(&p2, &b, &a);

    assert_eq!(o1.clone(), o2.clone());
}
...
fn lattice_meet_identity_all_ones() {
    ...
    let ones = [0xFFFF_FFFFu32];
    let program = vyre_libs_math::math::algebra::lattice_meet("a", "ones", "out", 1);
    let outputs = eval_pair(&program, &a, &ones);

    assert_eq!(decode_u32_words(&outputs), vec![0xDEAD_BEEF]);
}
...
fn lattice_meet_all_zeros_annihilates() {
    ...
    let zero = [0u32];
    let program = vyre_libs_math::math::algebra::lattice_meet("a", "zero", "out", 1);
    let outputs = eval_pair(&program, &a, &zero);

    assert_eq!(decode_u32_words(&outputs), vec![0]);
}

#[test]
fn lattice_meet_idempotent() {
    let a = [0xA5A5_A5A5u32];
    let program = vyre_libs_math::math::algebra::lattice_meet("a", "b", "out", 1);
    let outputs = eval_pair(&program, &a, &a);

    assert_eq!(decode_u32_words(&outputs), vec![0xA5A5_A5A5]);
}
...
fn semiring_min_plus_mul_basic() {
    ...
    let b = [1u32, 2, 3, 4];
    let program = vyre_libs_math::math::algebra::semiring_min_plus_mul("a", "b", "out", 4);
    let outputs = eval_pair(&program, &a, &b);

    assert_eq!(decode_u32_words(&outputs), vec![11, 22, u32::MAX, u32::MAX]);
}
