...
pub(super) struct VarScope {
    ...
    tile_bindings: imbl::HashMap<Ident, Vec<u32>>,
}

#[derive(Clone, Default)]
pub(super) struct ScopeSnapshot {
    pub(super) bindings: imbl::HashMap<Ident, u32>,
    pub(super) tile_bindings: imbl::HashMap<Ident, Vec<u32>>,
}
...
impl VarScope {
    pub(super) fn bind(&mut self, name: Ident, result: u32) -> Option<u32> {
        self.tile_bindings.insert(name.clone(), vec![result]);
        self.bindings.insert(name, result)
    }

    pub(super) fn bind_tile(&mut self, name: Ident, results: Vec<u32>) {
        if let Some(&first) = results.first() {
            self.bindings.insert(name.clone(), first);
        }
        self.tile_bindings.insert(name, results);
    }
    ...
    pub(super) fn get_tile(&self, name: &Ident) -> Option<Vec<u32>> {
        self.tile_bindings
            .get(name)
            .cloned()
            .or_else(|| self.bindings.get(name).map(|&id| vec![id]))
    }
