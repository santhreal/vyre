...
use crate::descriptor::{KernelBody, KernelOp, KernelOpKind, LiteralValue, OpaqueNodeData};
use crate::error::LowerError;
use rustc_hash::FxHashSet;
use vyre_foundation::ir::node_op_id;
use vyre_foundation::ir::{BinOp, DataType, Expr, Ident, Node, SubgroupReduceOp};
    ...
    fn lower_node(
        ...
        match node {
                    ...
                    for idx in 0..total_elements {
                        ...
                        for (i, &coord_c) in coords.iter().enumerate() {
                            ...
                            let term = if strides[i] == 1 {
                                ...
                                self.binary(body, KernelOpKind::BinOpKind(BinOp::Mul), c_offset, stride_lit)?
                            };
                            sum_id = match sum_id {
                                None => Some(term),
            ...
            Node::TileReduce { out, tile, op, axis } => {
                ...
                self.scope.bind_tile(out.clone(), out_extents, elem_type, res);
                Ok(())
            }
            Node::TileElementwise {
                ...
                let saved_bindings: Vec<(Ident, Option<super::scope::TileBinding>, Option<u32>)> = inputs
                    .iter()
                    .map(|name| (name.clone(), self.scope.get_tile(name), self.scope.get(name)))
                    .collect();
                ...
                for (name, tile_bind, scalar_bind) in saved_bindings {
                    if let Some(tb) = tile_bind {
                        self.scope.bind_tile(name, tb.extents, tb.element, tb.results);
