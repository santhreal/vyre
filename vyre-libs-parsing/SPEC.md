...
Layer `libraries`. Owner `libs-parsing`.

## Owns

Parsing compositions: lexer drivers, LR(1) table walkers, and the AST construction kernels a generated table drives.
...
5. A hand-written parser for one language embedded as control flow. A grammar is data this crate walks.
6. Pattern scanning. Substring, DFA, NFA, and regex are `vyre-libs-pattern`.
...
`vyre-foundation`, `vyre-primitives` and `vyre-spec` must never depend on `vyre-libs-parsing`. The edge is one way: a cycle back into this crate makes the two crates one crate that cannot be built, reviewed or published apart.
...
- The public surface is recorded in `docs/public-api/vyre-libs-parsing.txt`. An item added, removed or moved changes that file in the same change.
