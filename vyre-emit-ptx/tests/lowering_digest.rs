...
//! holds only while the stamp is live: a digest that misses a
//! source file, or a build the script no longer reruns for, pins the old value
//! and the caches go back to serving stale PTX. This recomputes the digest
...
fn the_stamped_lowering_digest_matches_the_emitter_source_on_disk() {
    ...
    assert_eq!(
        ...
        "Fix: the stamped PTX lowering digest is not the digest of the emitter source on disk, \
         ...
         wrote. Rebuild so build.rs reruns, and confirm it hashes every file under src."
    );
}
