...
fn buffer_length_registers_are_preloaded_before_branch_stores() {
    let kernel = descriptor("preload_lengths_before_branch_store")
        .slot(global_wo(0, DataType::U32, "out").with_count(16))
        .dispatch(16, 1, 1)
        .body(
            body()
                .ops([
                    ...
                    lit(1, 2),
                    lit(2, 3),
                    effect(KernelOpKind::StructuredIfThenElse, [1, 0, 1]),
                ])
                .children([
                    body().op(effect(KernelOpKind::StoreGlobal, [0, 0, 2])),
                    body().op(effect(KernelOpKind::StoreGlobal, [0, 0, 3])),
                ])
                .literals([
