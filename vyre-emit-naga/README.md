...
This crate is the `emitter` layer's `primary-text-emitter` seam, at `vyre-emit-naga`.
Every production dependency on it crosses Primary text emission and the related binary form for one selected module.
...
```console
./cargo_full run -p vyre-emit-naga --example vyre_emit_naga_release_surface
```
...
See [`docs/testing/vyre-emit-naga.md`](../docs/testing/vyre-emit-naga.md) for the crate's test command,
...
`vyre-emit-naga@0.8.0` is a publishable crate on the current Vyre release train. Publication still requires the release evidence and user-approval gates.
