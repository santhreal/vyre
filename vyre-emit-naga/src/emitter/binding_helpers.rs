...
use super::carrier_scope::LocalPool;
...
impl BodyBuilder<'_> {
    ...
    pub(super) fn value_handle_for_id(&mut self, id: u32) -> Option<naga::Handle<Expression>> {
        ...
        if let Some(local) = self.block_scoped_locals.get(&id).copied() {
            return Some(self.load_local_as(local, expected_ty));
        }
    ...
    pub(super) fn bind_result(
        ...
        // Where this result publishes from. Decided once: the publish itself,
        // the trace name, and the traced local type all read it.
        let pool = if self.loop_carrier_targets.contains(&result) {
            Some(LocalPool::LoopCarrier)
        } else if self.child_body_depth > 0 {
            Some(LocalPool::BlockScoped)
        } else {
            None
        };
        match pool {
            // Q7: when this id is loop-carried, Store to the carrier local so
            // the post-loop reader sees the final iteration's value, and
            // rebind `self.values[id]` to a fresh Load so a subsequent reader
            // inside the same iteration also reads the updated value rather
            // than the just-computed expression handle.
            Some(LocalPool::LoopCarrier) => {
                // Numeric descriptor ids are not globally unique after
                // structured lowering. A previous child block may have
                // allocated a block-scoped local for the same number, but this
                // bind is the current loop-carried SSA value. If the stale
                // block local stays in the map, value_handle_for_id resolves
                // future accumulator reads to the older block constant before
                // it ever reaches the live loop carrier. Sinkhorn's GEMM
                // accumulator hit exactly that: id 99 first denoted matrix
                // width `2`, then the accumulator snapshot, causing every sum
                // to start at 2. Drop the stale block shadow when a real
                // carrier owns the id.
                self.block_scoped_locals.remove(&result);
                if std::env::var("VYRE_PUBLISH_TRACE").is_ok() {
                    let underlying = self.resolve_underlying_local_kind(value);
                    let scalar = self.scalar_kind_of_expression(value, 0);
                    tracing::debug!(
                        "[publish] result={result} op={kind:?} init={value:?} scalar={scalar:?} underlying_local={underlying:?}",
                        kind = op.kind,
                    );
                }
                self.publish_through_local(LocalPool::LoopCarrier, result, value);
            }
            // Conservative block-scoping fix: any value produced inside a
            // child block (if-then arm, loop body, etc.) gets a function-scope
            // LocalVariable so it can be re-Loaded in a different block.
            // Without this, naga's expression-scoping validator rejects the
            // SSA handle as NotInScope when it is referenced from a block that
            // does not contain its Statement::Emit.
            Some(LocalPool::BlockScoped) => {
                self.publish_through_local(LocalPool::BlockScoped, result, value);
            }
        ...
        if let Ok(log_path) = std::env::var("VYRE_BIND_RESULT_LOG") {
            let entry = crate::BindResultEntry {
                ...
                publish_path: pool.map_or("Inline", LocalPool::trace_name).to_string(),
                local_allocated_ty: pool
                    .and_then(|pool| self.scoped_local(pool, result))
                    .map(|local| self.function.local_variables[local].ty.index() as u32),
            };
            use std::io::Write;
