...
Layer `emitter`. Owner `primary-text-emitter`.

## Owns

Consume verified lowering products and emit the primary text representation and
related binary targets.
...
- `vyre-debug` over the `primary-text-emitter` seam, public.
- `vyre-driver-wgpu` over the `primary-text-emitter` seam, private.
- `vyre-emit-metal` over the `primary-text-emitter` seam, private.
- `vyre-emit-spirv` over the `primary-text-emitter` seam, public.

## Direction that may not reverse

`vyre-foundation`, `vyre-lower` must never depend on `vyre-emit-naga`. The edge
...
- The crate declares no feature beyond `default`, so every build of it is the
  same build.
- The public surface is recorded in `docs/public-api/vyre-emit-naga.txt`. An
