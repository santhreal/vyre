...
fn main(@builtin(global_invocation_id) _vyre_global_id: vec3<u32>, @builtin(workgroup_id) _vyre_workgroup_id: vec3<u32>, @builtin(local_invocation_id) _vyre_local_id: vec3<u32>) {
    ...
    if false {
        ...
        if _e6 {
            ...
            let _e22 = vyre_block_scope_11_;
            if (_e22 < arrayLength((&out))) {
                out[_e16] = _e20;
            }
        } else {
            vyre_block_scope_12_ = 13u;
            vyre_block_scope_13_ = 0u;
            let _e35 = vyre_block_scope_13_;
            let _e39 = vyre_block_scope_12_;
            let _e41 = vyre_block_scope_13_;
            if (_e41 < arrayLength((&out))) {
                out[_e35] = _e39;
            }
        }
    } else {
        ...
        let _e54 = vyre_block_scope_21_;
        let _e58 = vyre_block_scope_20_;
        let _e60 = vyre_block_scope_21_;
        if (_e60 < arrayLength((&out))) {
            out[_e54] = _e58;
        }
    }
}
...
fn main(@builtin(global_invocation_id) _vyre_global_id: vec3<u32>, @builtin(workgroup_id) _vyre_workgroup_id: vec3<u32>, @builtin(local_invocation_id) _vyre_local_id: vec3<u32>) {
    if (_vyre_local_id.x < arrayLength((&out))) {
...
fn main(@builtin(global_invocation_id) _vyre_global_id: vec3<u32>, @builtin(workgroup_id) _vyre_workgroup_id: vec3<u32>, @builtin(local_invocation_id) _vyre_local_id: vec3<u32>) {
    ...
    let _e8 = u32_buf[0u];
    let _e9 = (_e5 + _e8);
    if (0u < arrayLength((&u32_buf))) {
        u32_buf[0u] = _e9;
    }
    let _e17 = f32_buf[0u];
    if (0u < arrayLength((&f32_buf))) {
        f32_buf[0u] = (_e17 + f32(_e9));
    }
}
...
fn main(@builtin(global_invocation_id) _vyre_global_id: vec3<u32>, @builtin(workgroup_id) _vyre_workgroup_id: vec3<u32>, @builtin(local_invocation_id) _vyre_local_id: vec3<u32>) {
    ...
    if (_vyre_local_id.x < arrayLength((&global_out))) {
...
fn main(@builtin(global_invocation_id) _vyre_global_id: vec3<u32>, @builtin(workgroup_id) _vyre_workgroup_id: vec3<u32>, @builtin(local_invocation_id) _vyre_local_id: vec3<u32>) {
    ...
    loop {
        ...
        let _e25 = vyre_block_scope_10_;
        if (_e25 < arrayLength((&out))) {
            out[_e19] = _e23;
        }
        continuing {
            let _e30 = i;
