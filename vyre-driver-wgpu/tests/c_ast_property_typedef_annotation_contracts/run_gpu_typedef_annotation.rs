...
use crate::c_ast_gpu_parity_support::{
    run_gpu_full_typedef_annotation, run_gpu_vast_builder_from_parts as run_gpu_vast_builder,
    starts_for_lens, word_at,
};
...
pub(crate) const C_TYPEDEF_FLAG_VISIBLE_TYPEDEF_NAME: u32 = 1;
pub(crate) const C_TYPEDEF_FLAG_TYPEDEF_DECLARATOR: u32 = 1 << 1;
pub(crate) const C_TYPEDEF_FLAG_ORDINARY_DECLARATOR: u32 = 1 << 2;
pub(crate) const VAST_TYPEDEF_FLAGS_FIELD: usize = 7;
pub(crate) const VAST_TYPEDEF_SCOPE_FIELD: usize = 8;
pub(crate) const VAST_TYPEDEF_SYMBOL_FIELD: usize = 9;

pub(crate) const VAST_STRIDE_BYTES: usize = NODE_STRIDE_U32 * 4;

pub(crate) fn run_gpu_typedef_annotation(
    raw_vast: &[u8],
    haystack: &[u8],
    num_nodes: u32,
) -> Vec<u8> {
...
pub(crate) fn build_fixture(atoms: &[Atom]) -> (Vec<u32>, Vec<u32>, Vec<u32>, Vec<u8>) {
...
pub(crate) fn assert_annotation_invariants(node_bytes: &[u8], _haystack: &[u8]) {
...
#[test]
pub(crate) fn typedef_simple_visible_name() {
    // typedef int T; T x;
    let atoms = vec![
...
#[test]
pub(crate) fn typedef_shadowing_nested_block() {
...
#[test]
pub(crate) fn typedef_struct_tag_vs_typedef_name() {
...
#[test]
pub(crate) fn typedef_many_in_sequence() {
...
#[test]
pub(crate) fn typedef_symbol_hash_matches_fnv1a() {
    // typedef int Foo;
    let atoms = vec![
        Atom::Tok(TOK_TYPEDEF),
        Atom::Tok(TOK_INT),
...
#[test]
pub(crate) fn gpu_parity_typedef_simple() {
    let atoms = vec![
        Atom::Tok(TOK_TYPEDEF),
        Atom::Tok(TOK_INT),
        Atom::Ident("T"),
...
#[test]
pub(crate) fn gpu_parity_typedef_shadowing() {
    let atoms = vec![
        Atom::Tok(TOK_TYPEDEF),
        Atom::Tok(TOK_INT),
        Atom::Ident("T"),
...
#[test]
pub(crate) fn gpu_parity_typedef_struct_tag() {
    let atoms = vec![
        Atom::Tok(TOK_TYPEDEF),
        Atom::Tok(TOK_STRUCT),
        Atom::Ident("S"),
