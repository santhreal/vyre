//! Contract tests for C cast/declaration contexts, redeclarations, and struct fields.
//!
//! C11 semantics:
//!   * A typedef name in a cast `(T)` introduces a cast expression; `*` after it is unary deref
//!   * A typedef name in a declaration `T * p;` introduces a pointer declarator
//!   * Redeclaration in the same scope is invalid (but parser may or may not flag it)
//!   * A declaration in an inner block shadows an outer one
//!   * struct field names are in a separate namespace and do not leak
...
use vyre_libs::parsing::c::parse::vast::{
    ...
    C_AST_KIND_FUNCTION_DECLARATOR, C_AST_KIND_POINTER_DECL,
};
use vyre_libs::parsing::c::sema::lookup::{
    DECL_KIND_FUNCTION, DECL_KIND_FUNCTION_DECL, DECL_KIND_NONE, DECL_KIND_VARIABLE,
};
...
mod c_ast_sema_scope_cast_decl_redecl_field_contracts_classifier_typedef_in_cast_is_cast_expr {

    include!("contract_cases/c_ast_sema_scope_cast_decl_redecl_field_contracts__classifier_typedef_in_cast_is_cast_expr.rs");
}
mod c_ast_sema_scope_cast_decl_redecl_field_contracts_scope_tree_struct_field_not_classified_as_declaration {
    include!("contract_cases/c_ast_sema_scope_cast_decl_redecl_field_contracts__scope_tree_struct_field_not_classified_as_declaration.rs");
}
