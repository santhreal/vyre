...
fn concurrent_compile_and_save_one_wins_neither_corrupts() {
    ...
    for (_, engine) in &results {
        assert_eq!(engine.reference_scan(b"concurrent"), vec![ByteRange::new(0, 0, 10)]);
    }
    ...
    assert_eq!(loaded.reference_scan(b"concurrent"), vec![ByteRange::new(0, 0, 10)]);
}
    ...
    let reader = {
        ...
        thread::spawn(move || {
            ...
            for _ in 0..50 {
                ...
                let is_old = old_matches == vec![ByteRange::new(0, 0, 3)];
                let is_new = new_matches == vec![ByteRange::new(0, 0, 11)];
                assert!(
                    is_old || is_new,
...
fn concurrent_stale_delete_and_recompile() {
    ...
    for handle in handles {
        ...
        assert_eq!(engine.reference_scan(b"fresh"), vec![ByteRange::new(0, 0, 5)]);
    }
}
...
fn concurrent_stress_no_panic_with_many_threads() {
    ...
    for handle in handles {
        ...
        assert_eq!(matches, vec![ByteRange::new(0, 0, 6)]);
    }
}
