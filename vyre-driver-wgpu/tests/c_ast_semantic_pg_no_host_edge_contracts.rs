...
use c_ast_gpu_parity_support::{
    ...
    classify, first_row, node_count_from_vast, row_indices,
...
fn fixture_typedef_struct_enum_fnptr() -> Fixture {
    c_tokens(
        "typedef int T ; struct S { int x ; } ; enum E { A , B } ; void ( * fp ) ( struct S * ) ;",
    )
}
...
fn fixture_switch_case_default_goto_label() -> Fixture {
    c_tokens(
        "void f ( int x ) { switch ( x ) { case 1 : break ; default : goto end ; } end : return ; \
         }",
    )
}
