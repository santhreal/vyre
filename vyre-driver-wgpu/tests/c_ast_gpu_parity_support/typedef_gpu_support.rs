...
pub(crate) fn run_gpu_full_typedef_annotation(source: &[u8], raw_vast: &[u8]) -> Vec<u8> {
    ...
    let program = program::annotate_typedefs(source.len() as u32, node_count_from_vast(raw_vast));
...
pub(crate) fn run_gpu_fast_typedef_annotation(source: &[u8], raw_vast: &[u8]) -> Vec<u8> {
    ...
    let hashed_program = program::prehash_identifiers(source.len() as u32, node_count_value);
    ...
    let scoped_program = program::precompute_scopes(node_count_value);
    ...
    let scoped_vast = primary_output(scoped, "GPU typedef scope precompute");
...
/// Annotate a VAST on the GPU the way `reference_c11_annotate_typedef_names`
/// does, through the shared three-dispatch scoped sequence.
///
/// Binds this crate's [`GpuArm`] to `arm_annotated_vast`, which owns the
/// sequence because the CPU-reference arm in `vyre-libs/tests` runs it too.
///
/// It has to be the SCOPE-AWARE annotator. The global-fast path resolves a name
/// by testing its hash against the set of file-scope typedefs and has no scope
/// model at all, so it cannot represent shadowing in either direction, and both
/// directions showed up as parity failures:
///
...
/// `c_global_typedef_annotate_parity` in `vyre-libs/tests` pins what it does
/// and does not promise.
pub(crate) fn run_gpu_scoped_typedef_annotation(source: &[u8], raw_vast: &[u8]) -> Vec<u8> {
    arm_annotated_vast(&GpuArm, source, raw_vast)
}
