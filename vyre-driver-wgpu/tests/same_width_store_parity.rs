...
fn dispatch_words(backend: &WgpuBackend, program: &Program, a: &[u32], b: &[u32]) -> Vec<u32> {
    ...
    let outputs = backend
        .dispatch_borrowed(
            ...
            &[
                out_init.as_slice(),
                u32_bytes(a).as_slice(),
                u32_bytes(b).as_slice(),
            ],
...
fn i32_value_stored_into_u32_buffer_preserves_bits_on_gpu() {
    let backend =
        WgpuBackend::acquire().expect("Fix: same-width store parity requires a live GPU backend.");
    ...
    assert_eq!(
        gpu[0] as i32, -3,
        "5 - 8 must read back as -3 from the u32 buffer"
    );
}
...
fn u32_value_stored_into_i32_buffer_preserves_bits_on_gpu() {
    ...
    assert_eq!(
        gpu[2] as i32,
        i32::MIN,
        "0x80000000 must survive as i32::MIN"
    );
}
