...
pub(crate) struct UnifiedEntry {
    ...
    pub(crate) test_inputs: Option<fn() -> Vec<Vec<Vec<u8>>>>,
}
...
pub(crate) fn entry_by_index(idx: usize) -> &'static UnifiedEntry {
...
pub(crate) fn gpu() -> &'static WgpuBackend {
...
pub(crate) fn validate_for_backend(program: &Program) -> Result<(), String> {
...
pub(crate) fn missing_capability_reason(program: &Program) -> Option<String> {
...
pub(crate) fn rename_buffer_in_expr(expr: &Expr, old: &str, new: &str) -> Expr {
    match expr {
...
pub(crate) fn rename_buffer_in_node(node: &Node, old: &str, new: &str) -> Node {
    match node {
...
pub(crate) fn rename_buffer_in_program(prog: &Program, old: &str, new: &str) -> Program {
...
pub(crate) fn demote_output_to_intermediate(prog: &Program, name: &str) -> Program {
    let buffers: Vec<BufferDecl> = prog
        .buffers()
        .iter()
        .map(|buf| {
...
pub(crate) fn needs_input(buf: &BufferDecl) -> bool {
...
pub(crate) fn try_compose(a: &UnifiedEntry, b: &UnifiedEntry) -> Result<Composition, String> {
    let prog_a = (a.build)();
    let prog_b = (b.build)();
