...
//! `OptimizerDispatcher`. Satisfying that trait from a live `WgpuBackend` is one
...
/// expects.
pub(crate) struct WgpuOptimizerDispatcher<'a> {
...
impl OptimizerDispatcher for WgpuOptimizerDispatcher<'_> {
    fn dispatch(
        &self,
        program: &Program,
        inputs: &[Vec<u8>],
