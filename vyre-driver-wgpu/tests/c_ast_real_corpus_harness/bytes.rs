...
pub(crate) fn run_reference_eval(program: &vyre::ir::Program, inputs: &[Vec<u8>]) -> Vec<Vec<u8>> {
...
pub(crate) fn run_harness(name: &str, source: &str, tokens: &[(&str, u32)]) {
    ...
    let vast_gpu_outputs = dispatch_gpu_program(
        "real corpus VAST build",
        prog_vast.clone(),
        vast_inputs.clone(),
    );
    ...
    let pg_gpu_outputs =
        dispatch_gpu_program("real corpus PG lower", prog_pg.clone(), pg_inputs.clone());
    // Binding 1 is ReadWrite. [0] is binding 1.
    ...
    assert_parity(
        &pg_reference_eval,
        &pg_cpu_ref,
        "PG Lower (Reference Eval)",
        name,
    );
}

#[test]
pub(crate) fn test_kernel_atomic_add_unless_parity() {
...
#[test]
pub(crate) fn test_libc_qsort_parity() {
...
#[test]
pub(crate) fn test_sqlite_malloc_parity() {
