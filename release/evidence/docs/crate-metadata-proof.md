...
- Publishable Vyre crates must be version `0.7.0`.
- Publishable Weir crates must be version `0.1.1`.
- `vyre-frontend-c` must be included as a versioned `0.7.0` non-publishable release-surface crate with `README.md`; `publish=false` is intentional for this release and does not waive metadata quality.
...
- CUDA and WGPU driver crates must be classified as required `0.7.0` publishable backend release surfaces (`cuda-backend` and `wgpu-backend`), not as generic internal Vyre crates.
