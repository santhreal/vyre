...
- `metadata-matrix.json` must report an empty `missing_required_release_surfaces` array, proving `vyre`, `vyre-driver-cuda`, and `vyre-driver-wgpu` are present with the expected versions, release kinds, release surfaces, and README metadata.
...
- `feature-matrix.json` must report an empty `missing_required_release_packages` array covering `vyre`, CUDA, and WGPU.
