...
- `docs/testing/THESIS_AXES.toml` must state compound release axes for parsing throughput, dataflow convergence, graph traversal, pattern matching, fixpoint witness propagation, persistent megakernel batches, and optimizer saturation. Each axis must resolve to release-suite macro workloads that require GPU execution, carry CUDA-bound CPU-SOTA baseline contracts, and operate at at least 1 MiB input scale. The contract is hand-authored: deriving the axes from the registry would compare the registry with itself.
