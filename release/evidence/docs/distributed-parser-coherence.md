...
- `vyre-frontend-c`, `vyrec`, Weir, and Vyre grammar generation must all have explicit parser ownership contracts.
...
- `distributed-parser-map.json` must expose required test-category coverage for `vyre-frontend-c`, `vyrec`, Weir, and Vyre grammar generation so distributed parser ownership cannot pass on README terms alone; category evidence must include component `tests/`, `benches/`, and `fuzz/` trees.
