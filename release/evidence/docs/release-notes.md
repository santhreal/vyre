...
- Release notes must state that `vyre`, `vyre-driver-cuda@0.7.2`, and `vyre-driver-wgpu@0.7.2` are present on the `0.7.2` Vyre release train; `missing_required_release_packages` in `version-matrix.json` must be empty before notes are cut.
