...
- Release notes must use `Vyre 0.7.2`.
- Release notes must state that `vyre`, `vyre-driver-cuda@0.7.2`, `vyre-driver-wgpu@0.7.2`, and `vyre-frontend-c` are present on the `0.7.2` Vyre release train; `missing_required_release_packages` in `version-matrix.json` must be empty before notes are cut.
...
- Release notes must reference RC tag `vyre-v0.7.2-rc.1` before final tag `vyre-v0.7.2`.
- Release notes must not instruct a bare `v0.7.2` tag workflow.
