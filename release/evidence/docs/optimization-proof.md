...
- The external-fact-aware DSE family must exist and must fire under imported facts.
- Alias/reaching-definition-aware DSE, STLF, LICM, loop fusion, and loop fission must be wired into the canonical optimization pipeline or explicit loop transform APIs.
- Integration markers must prove that external facts fire DSE, STLF, LICM, loop fusion, and loop fission transformations, plus the reaching-definition Copy-chain canonicalization used by those legality checks.
- Cross-binding memory accesses are conservative by default; explicit `NoAliasFact` evidence is required to recover DSE, STLF, LICM, loop fusion, and loop fission across distinct global bindings.
