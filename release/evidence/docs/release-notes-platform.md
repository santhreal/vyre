...
- Release notes must use `vyre 0.7.1` and `weir 0.1.2`.
- Release notes must state that `vyre`, `vyre-driver-cuda@0.7.1`, `vyre-driver-wgpu@0.7.1`, `vyrec`, and `vyre-frontend-c` are present on the `0.7.1` Vyre release train, and `weirflow@0.1.2` is present on the Weir release train; `missing_required_release_packages` in `version-matrix.json` must be empty before notes are cut.
...
- Release notes must reference RC tags `vyre-v0.7.1-rc.1`, `weir-v0.1.2-rc.1`, and `vyre-0.7.1-weir-0.1.2-rc.1` before final tags `vyre-v0.7.1`, `weir-v0.1.2`, and `vyre-0.7.1-weir-0.1.2`.
- Release notes must not instruct a bare `v0.7.1` tag workflow.
