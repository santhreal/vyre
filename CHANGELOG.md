...
- The in-memory and on-disk pipeline caches ask the same question about a
  ...
  `disk_cache_entries.rs`. `runtime/device/device.rs` repeated its parent and
  named nothing; it owns device acquisition, the process-wide cached device,
  feature negotiation against the adapter, and the host waits acquisition
  needs, so it is `runtime/device/acquire.rs`.
