...
- The `vyre-foundation` duplication pin sits at what the tree measures, 5125
  down to 4350 duplicated lines over 160 files, with the total line count
  corrected from a stale 95984 to a measured 94924. A pin with room under it
  hides the next copy.
...
- Program capability scanning and backend capability enforcement are proved in
  ...
  module in `src/program_caps.rs` exercised the same public surface and the two
  had drifted in opposite directions. The suite derives its expectations from
  the public API snapshot at run time: every boolean `RequiredCapabilities`
  field must have a scan case that a program needing the feature sets and a
  scalar program does not, and every `supports_*` parameter of
  `check_backend_capabilities` must have a case where withholding it rejects a
  program that needs it, so a capability added later fails the suite until
  somebody records what proves it.
- The `vyre-foundation` duplication pin records what the tree measures, 4127 to
  3679 duplicated lines, with `total_lines` measured. A pin with room under it
  hides the next copy.
