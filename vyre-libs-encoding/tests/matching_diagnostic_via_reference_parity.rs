...
use vyre_libs_encoding::encoding::matching_diagnostic_compaction::{
    bracket_pairs_via, dedup_region_survivor_flags_via, sort_regions_via,
};
use vyre_libs_pattern::pattern::{RegionTriple, BRACKET_KIND_CLOSE, BRACKET_KIND_OPEN};
...
fn bracket_pairs_via_matches_known_nested_pairs() {
    ...
    let pairs =
        bracket_pairs_via(&dispatcher, &semantic_requests::wrapper_policy(), &kinds, 8).unwrap();
...
fn sort_regions_via_matches_cpu_sort_over_generated_batches() {
    ...
    for case in 0..300u32 {
        ...
        let sorted = sort_regions_via(&dispatcher, &semantic_requests::wrapper_policy(), &regions)
