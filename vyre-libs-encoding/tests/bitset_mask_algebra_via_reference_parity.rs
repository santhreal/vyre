...
//! [`vyre_driver_reference::ReferenceSemanticExecutor`], across every mask operation the consumer exposes:
...
use vyre_libs_encoding::encoding::bitset_mask_algebra::{
...
fn word_parallel_and_or_xor_not_via_match_cpu_ref() {
    ...
    for case in 0..400u32 {
        ...
        assert_eq!(
            mask_not_via(&dispatcher, &semantic_requests::wrapper_policy(), &lhs).unwrap(),
...
fn bitset_mask_via_hand_checked_cases() {
    ...
    assert_eq!(
        mask_and_via(&d, &semantic_requests::wrapper_policy(), &a, &b).unwrap(),
        vec![0b1000, 0x0F00_000F],
        "AND"
    );
    assert_eq!(
        mask_or_via(&d, &semantic_requests::wrapper_policy(), &a, &b).unwrap(),
        vec![0b1110, 0xFF0F_0FFF],
        "OR"
    );
    assert_eq!(
        mask_xor_via(&d, &semantic_requests::wrapper_policy(), &a, &b).unwrap(),
        vec![0b0110, 0xF00F_0FF0],
        "XOR"
    );
    assert_eq!(
        mask_not_via(&d, &semantic_requests::wrapper_policy(), &a).unwrap(),
        vec![!0b1100u32, !0xFF00_00FFu32],
        "NOT"
    );

    assert!(
        mask_equal_via(&d, &semantic_requests::wrapper_policy(), &a, &a).unwrap(),
        "a == a"
    );
    assert!(
        !mask_equal_via(&d, &semantic_requests::wrapper_policy(), &a, &b).unwrap(),
        "a != b"
    );

    // bit 3 (value 0b1000) is set in a[0]=0b1100? 0b1100 has bits 2,3 → bit 3 set.
    assert!(
        mask_test_bit_via(&d, &semantic_requests::wrapper_policy(), &a, 3).unwrap(),
        "bit 3 set in 0b1100"
    );
    assert!(
        !mask_test_bit_via(&d, &semantic_requests::wrapper_policy(), &a, 0).unwrap(),
        "bit 0 clear in 0b1100"
    );

    // set bit 0 in a → a[0] becomes 0b1101.
    let set = mask_set_bit_via(&d, &semantic_requests::wrapper_policy(), &a, 0).unwrap();
    assert_eq!(set[0], 0b1101, "set_bit(0) turns on bit 0");
    // clear bit 2 in a → a[0] becomes 0b1000.
    let cleared = mask_clear_bit_via(&d, &semantic_requests::wrapper_policy(), &a, 2).unwrap();
