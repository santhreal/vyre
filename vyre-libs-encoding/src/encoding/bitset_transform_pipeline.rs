    ...
    fn program_builders_emit_expected_bitset_primitives() {
        let cases = [
            (
                ...
                vyre_libs_bitset::bitset::and_not::OP_ID,
            ),
            (
                narrow_mask_in_place_program("target", "mask", 2),
                vyre_libs_bitset::bitset::and_into::OP_ID,
            ),
            (
                grow_mask_in_place_program("target", "addend", 2),
                vyre_libs_bitset::bitset::or_into::OP_ID,
            ),
            (
                diff_mask_in_place_program("target", "addend", 2),
                vyre_libs_bitset::bitset::xor_into::OP_ID,
            ),
            (
                subtract_mask_in_place_program("target", "sub", 2),
                vyre_libs_bitset::bitset::and_not_into::OP_ID,
            ),
            (
                copy_mask_program("target", "source", 2),
                vyre_libs_bitset::bitset::copy::OP_ID,
            ),
            (
                any_mask_program("input", "out", 2),
                vyre_libs_bitset::bitset::any::OP_ID,
            ),
            (
                select1_navigation_program("bits", "queries", "out", 2, 2),
                vyre_libs_bitset::bitset::select::OP_ID,
            ),
            (
                four_russians_transform_program("lhs", "rhs", "lut", "out", 2),
                vyre_libs_bitset::bitset::four_russians::OP_ID,
            ),
            (
                four_russians_dense_matvec_program("frontier", "tile_lut", "out", 2, 2),
                vyre_libs_bitset::bitset::four_russians::DENSE_MATVEC_OP_ID,
            ),
        ];

        for (program, expected) in cases {
            let actual = vyre_libs_builder::test_parity_oracles::region_operation_id(&program)
