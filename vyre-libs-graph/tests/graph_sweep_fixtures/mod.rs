...
//!
//! The frontier word count, the one-round frontier-step runner and the
//! generated-CSR family have one home here, so a case that fails in one suite
//! is the same graph another suite passed.
...
pub(crate) type FrontierStepBuilder = fn(
    vyre_libs_graph::graph::program_graph::ProgramGraphShape,
    &str,
    &str,
    u32,
