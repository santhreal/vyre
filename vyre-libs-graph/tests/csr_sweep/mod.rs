...
/// The generator every group draws from, re-exported so a sweep matrix names one
/// module.
pub use vyre_test_support::sweep_rng::Rng;

use vyre_libs_graph::graph::csr_closure_inputs::{CsrClosureInputs, CsrGraphView};
...
pub type CsrSweepParts = (u32, Vec<u32>, Vec<u32>, Vec<u32>, Vec<u32>, u32);

impl CsrSweepCase {
    /// Consume this case into [`CsrSweepParts`].
    pub fn into_parts(self) -> CsrSweepParts {
        (
            self.node_count,
            self.offsets,
            self.targets,
    ...
    pub fn view(&self) -> CsrGraphView<'_> {
        CsrGraphView {
    ...
    pub fn inputs(&self, max_iters: u32) -> CsrClosureInputs<'_> {
        CsrClosureInputs {
            graph: self.view(),
...
pub fn declared_groups() -> &'static [CsrSweepGroup] {
    &[
        CsrSweepGroup {
...
pub fn group(name: &str) -> &'static CsrSweepGroup {
    declared_groups()
        .iter()
...
pub fn generate(group: &CsrSweepGroup, seed: u64) -> CsrSweepCase {
...
pub fn assert_every_group_is_swept(package: &str) {
    ...
    assert!(
        ...
        "Fix: {package} declares no sweep for CSR shape group(s) {unswept:?}. Either draw them \
         ...
         vyre-test-support/src/csr_sweep.rs so no crate claims coverage nobody has."
    );
}
...
pub fn oracle_persistent_closure(
    node_count: u32,
    edge_offsets: &[u32],
    edge_targets: &[u32],
    edge_kind_mask: &[u32],
