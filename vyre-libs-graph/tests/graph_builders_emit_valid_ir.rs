...
fn csr_queue_delta_enqueue_emits_valid_ir() {
    let program = vyre_libs_graph::graph::csr_queue_delta::csr_queue_delta_enqueue(
        "aq",
        "al",
        "eo",
        "et",
...
fn csr_queue_split_low_forward_traverse_emits_valid_ir() {
    let program = vyre_libs_graph::graph::csr_queue_split::csr_queue_split_low_forward_traverse(
        "aq",
        "ql",
        "eo",
        "et",
...
fn matroid_exchange_bfs_step_emits_valid_ir() {
    let program = vyre_libs_graph::graph::matroid::matroid_exchange_bfs_step(
        "frontier_in",
        "exchange_adj",
        "visited",
        "frontier_out",
...
fn intervention_delete_incoming_emits_valid_ir() {
    let program = vyre_libs_graph::graph::do_calculus::intervention_delete_incoming(
        "adjacency",
        "intervention_mask",
        "out_adjacency",
        4,
...
fn rule2_reverse_incoming_emits_valid_ir() {
    ...
    let program = vyre_libs_graph::graph::do_calculus::rule2_reverse_incoming(
        "adjacency",
        "treatment_mask",
        "out_adjacency",
        4,
...
fn backdoor_descendants_check_emits_valid_ir() {
    let program = vyre_libs_graph::graph::adjustment_set::backdoor_descendants_check(
        "candidate_z",
        "descendants_of_x",
        "out_violation",
        4,
...
fn persistent_bfs_emits_valid_ir() {
    ...
    let program = vyre_libs_graph::graph::persistent_bfs::persistent_bfs(
...
fn persistent_bfs_batch_emits_valid_ir() {
    ...
    let program = vyre_libs_graph::graph::persistent_bfs::persistent_bfs_batch(
        ProgramGraphShape::new(4, 4),
        "frontier_in",
        "frontier_out",
        "changed",
...
fn toposort_program_emits_valid_ir() {
    ...
    let program = vyre_libs_graph::graph::toposort::toposort_program(
        4, // node_count
        "offsets",
        "targets",
        "indeg_scratch",
...
fn chebyshev_filter_emits_valid_ir() {
    let program = vyre_libs_graph::graph::chebyshev_filter::chebyshev_filter(
        "laplacian",
        "signal",
        "coeffs",
        "output",
