...
//! | `vyre-runtime/src/resident_work_queue/planner/mod.rs` | **vyre's megakernel scheduler**  -  fusion-grouping subject to memory + sync constraints IS a matroid intersection problem (graphic matroid × partition matroid) |
...
pub fn try_matroid_exchange_bfs_step(
    ...
    let dense_cells = vyre_libs_builder::plumbing::operand::shape::square_matrix_cells(OP_ID, n)?;
