...
Layer `libraries`. Owner `libs-graph`.

## Owns

Graph algorithms as compositions: CSR traversal, AST walks, dominator trees, topological sort, and the topology records they consume.
...
5. A graph representation duplicated from `vyre-foundation`. The IR graph has one owner.
6. A traversal that materializes every frontier on the host to drive the next step.
...
- `vyre-libs-visual` over the `libs-visual` seam.
...
`vyre-foundation`, `vyre-primitives` and `vyre-spec` must never depend on `vyre-libs-graph`. The edge is one way: a cycle back into this crate makes the two crates one crate that cannot be built, reviewed or published apart.
...
- The public surface is recorded in `docs/public-api/vyre-libs-graph.txt`. An item added, removed or moved changes that file in the same change.
