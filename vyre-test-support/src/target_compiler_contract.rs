...
//! backend crate calls into this one owner from its own suite.
...
pub fn single_lane_artifact(access: BufferAccess, facts_seed: u8) -> Artifact {
    ...
    graph
        ...
        .expect("single-lane fixture graph must accept its one node. Fix: correct the program or the output this fixture declares for `main`");
    crate::artifact_fixtures::compile_graph(graph, facts_seed)
}
...
pub fn two_stage_artifact(access: BufferAccess) -> Artifact {
    ...
    let (_, first_values) = graph
        ...
        .expect("two-stage fixture graph must accept its producer. Fix: correct the producer's program or the contract of the value it publishes");
    let middle = *first_values
        .first()
        .expect("the producer declares one output value. Fix: keep the producer's single GraphOutput above, which this stage reads as `middle`");
    graph
        .add_node(
            "second",
        ...
        .expect("two-stage fixture graph must accept its consumer. Fix: correct the consumer's program or the contract it reads `middle` under");
    crate::artifact_fixtures::compile_graph(graph, 0)
}
...
pub fn registration(backend_id: &str) -> &'static BackendRegistration {
    let enumerated = vyre_driver::registered_backends()
        .expect("backend registry must build. Fix: correct the duplicate or malformed registration the driver crates publish")
        .iter()
...
impl TargetExpectation<'_> {
    ...
    pub fn foreign_artifact(&self) -> Artifact {
    ...
    pub fn compiled(&self) -> (Artifact, TargetPayload) {
    ...
    pub fn compiled_two_stage(&self) -> (Artifact, TargetPayload) {
        let compiler = registration(self.backend_id)
            .target_compiler()
            .unwrap_or_else(|error| {
                panic!(
...
pub fn assert_target_compiler_emits_bundle(
...
pub fn assert_materializer_executes_payload(expectation: &TargetExpectation<'_>) {
    let backend_id = expectation.backend_id;
    let (artifact, payload) = expectation.compiled();
...
pub fn assert_materializer_executes_resident_binding(expectation: &TargetExpectation<'_>) {
    let backend_id = expectation.backend_id;
    let (artifact, payload) = expectation.compiled();
    let materializer = materializer_for(backend_id);
    let instance = materializer
