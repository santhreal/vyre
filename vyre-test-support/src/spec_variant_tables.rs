...
//!
//! Every consumer reads these tables from this crate, the same way
//! `crate::artifact_fixtures` is shared. The assertion each suite makes about a
//! variant stays in that suite: this module owns only which variants exist.
//!
//! The `DataType` element table is not here. `DataType` fixtures are owned by
//! `crate::data_type_variants`, which holds them to the enum
//! declaration in `vyre-spec` at run time, and this module owns the operator
//! enums only.
//!
...
///
/// The member set is `crate::bin_op_variants`, which holds its
...
pub fn builtin_bin_ops() -> Vec<BinOp> {
    let mut builtins: Vec<(u8, BinOp)> = crate::bin_op_variants::bin_op_variant_samples()
        .into_iter()
...
pub fn builtin_atomic_ops() -> Vec<AtomicOp> {
    vec![
        AtomicOp::Add,
        AtomicOp::Or,
        AtomicOp::And,
...
pub fn buffer_data_types(element_size: usize) -> Vec<DataType> {
    crate::data_type_elements::flat_buffer_element_types(element_size)
}
...
pub fn public_api_variant_names(enum_name: &str) -> BTreeSet<String> {
    const PACKAGE: &str = "vyre-spec";
    let prefix = format!("pub vyre_spec::{enum_name}::");
    let names: BTreeSet<String> = crate::public_api::snapshot_text(PACKAGE)
        .lines()
    ...
    assert!(
        ...
        crate::public_api::snapshot_path(PACKAGE).display()
    );
    names
}

/// Every builtin `CollectiveOp`, in wire-tag order.
pub fn builtin_collective_ops() -> [(vyre_spec::CollectiveOp, u8); 6] {
    [
