...
///
/// Resolve `crate_dir` at run time, as
/// `vyre_test_support::monorepo::vyre_workspace_root().join(crate_name)`. A
/// directory fixed at compile time names whichever checkout built the binary,
/// so the gate then enumerates that tree's builders while reporting on this one.
