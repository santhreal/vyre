// Detection cases for the `raw_ir_in_libs` lint: what it must flag and what
// it must leave alone.
//
// Each case writes a synthetic vyre-libs source file to a tempdir, runs the
// lint, and asserts on the exact violation set.

#[path = "raw_ir_in_libs__evasion_cases.rs"]
mod raw_ir_in_libs_evasion_cases;
...
/// Lint a workspace holding exactly one synthetic `vyre-libs` source file.
/// Owns the tempdir so a case states only the source under test.
fn lint_one(rel: &str, source: &str) -> Vec<Violation> {
    let dir = tempfile::tempdir().unwrap();
    write_lib_file(dir.path(), rel, source);
    lint(dir.path())
}
