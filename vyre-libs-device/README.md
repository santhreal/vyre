...
The `product-libraries` owner maintains this `libraries` crate at `vyre-libs-device`.
...
Run the checked-in behavior from `vyre-libs-device/tests/all_tests.rs`:

```console
./cargo_full test -p vyre-libs-device --test all_tests
```

### Features

- Manifest features: `default`, `device`
...
See [`docs/testing/vyre-libs-device.md`](../docs/testing/vyre-libs-device.md) for the crate's test command,
...
`vyre-libs-device@0.8.0` is a publishable crate on the current Vyre release train. Publication still requires the release evidence and user-approval gates.
