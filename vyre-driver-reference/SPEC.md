...
Layer `concrete-backend`. Owner `reference-driver`.

## Owns

Adapt the reference interpreter to the backend contract for deterministic
...
- `vyre-bench` over the `reference-driver` seam, private.
- `vyre-registry-link` over the `reference-driver` seam, private.

## Direction that may not reverse

`vyre-driver`, `vyre-foundation`, `vyre-reference` must never depend on
`vyre-driver-reference`. The edge is one way: a cycle back into this crate
...
- The crate declares no feature beyond `default`, so every build of it is the
  same build.
- The public surface is recorded in
  `docs/public-api/vyre-driver-reference.txt`. An item added, removed or moved
  changes that file in the same change.
