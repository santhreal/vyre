...
Layer `libraries`. Owner `libs-bitset`.

## Owns

Packed `u32` bitset compositions and the logical bitwise operations over them: word utilities, set algebra, and population counts expressed as IR.
...
5. A bitset width or word size chosen from a device fact. The packing is a data-layout decision the caller states.
6. A second copy of a reduction. Population counts compose `vyre-libs-reduce`.
...
`vyre-foundation`, `vyre-primitives` and `vyre-spec` must never depend on `vyre-libs-bitset`. The edge is one way: a cycle back into this crate makes the two crates one crate that cannot be built, reviewed or published apart.
...
- The public surface is recorded in `docs/public-api/vyre-libs-bitset.txt`. An item added, removed or moved changes that file in the same change.
