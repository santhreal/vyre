//! Security-to-external IFDS routing contract tests.
//!
//! Gated by the same `external_ifds_engine` feature as the code it
//! exercises (`vyre_libs::security::external_ifds`, see security/mod.rs). That
...
//! wrong: `security` is enabled (e.g. by xtask) while the external_ifds
...
fn source_to_sink_query_dispatches_through_external_ifds_and_returns_witness_seed() {
    ...
    let request = SourceToSinkFindingRequest {
        ...
        reason: "route through external IFDS".to_string(),
    };

    let dispatch = route_security_taint_through_external_ifds(&table, &request, shape, buffers)
        .expect("Fix: fact-backed source-to-sink query should route through external IFDS");

    assert_eq!(dispatch.query_id, EXTERNAL_IFDS_GPU_OP_ID);
    assert_eq!(dispatch.backend_id, EXTERNAL_IFDS_SECURITY_BACKEND_ID);
    ...
    assert_eq!(
        routed, direct,
        "Fix: security routing must call the same external IFDS step builder"
    );
}

#[test]
fn external_witness_path_attaches_rule_spans_edge_kinds_soundness_and_source_bytes() {
    ...
    let bundle = FindingProofBundle {
        ...
        evidence_digest: "evidence:external-path".to_string(),
        ...
        confidence_bps: 10_000,
        reason: "external path proves source reaches sink".to_string(),
    };
    ...
    let witness = security_witness_path_from_external_path(
        &bundle,
        "c.external.source-to-sink",
        &path,
        &edge_kinds,
        &source_files,
    )
    .expect("Fix: external extracted path should attach to fact-backed finding");

    assert_eq!(witness.finding_id, "finding.security.external-ifds");
    assert_eq!(witness.rule_id, "c.external.source-to-sink");
    assert_eq!(witness.query_id, EXTERNAL_IFDS_GPU_OP_ID);
    assert_eq!(witness.backend_id, EXTERNAL_IFDS_SECURITY_BACKEND_ID);
