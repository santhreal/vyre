...
fn rank_superblocks_carry_across_more_blocks_than_lanes() {
    ...
    let bits: Vec<u32> = (0..300u32)
        ...
        .collect();
    let program =
        vyre_libs::math::succinct::rank1_superblocks("bits", "superblocks", bits.len() as u32, 1);
    let outputs = vyre_reference::reference_eval(
        &program,
        &[
            Value::from(u32_bytes(&bits)),
            Value::from(vec![0u8; (bits.len() + 1) * core::mem::size_of::<u32>()]),
        ],
    )
    .expect("rank1_superblocks must execute past one lane pass");
