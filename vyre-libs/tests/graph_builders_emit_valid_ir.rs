...
fn matroid_exchange_bfs_step_emits_valid_ir() {
    let program = vyre_libs::graph::matroid::matroid_exchange_bfs_step(
        "frontier_in",
        "exchange_adj",
        "visited",
        "frontier_out",
...
fn backdoor_descendants_check_emits_valid_ir() {
    let program = vyre_libs::graph::adjustment_set::backdoor_descendants_check(
        "candidate_z",
        "descendants_of_x",
        "out_violation",
        4,
...
fn persistent_bfs_emits_valid_ir() {
    ...
    let program = vyre_libs::graph::persistent_bfs::persistent_bfs(
...
fn persistent_bfs_batch_emits_valid_ir() {
    ...
    let program = vyre_libs::graph::persistent_bfs::persistent_bfs_batch(
        ProgramGraphShape::new(4, 4),
        "frontier_in",
        "frontier_out",
        "changed",
...
fn toposort_program_emits_valid_ir() {
    ...
    let program = vyre_libs::graph::toposort::toposort_program(
        4, // node_count
        "offsets",
        "targets",
        "indeg_scratch",
...
fn chebyshev_filter_emits_valid_ir() {
    let program = vyre_libs::graph::chebyshev_filter::chebyshev_filter(
        "laplacian",
        "signal",
        "coeffs",
        "output",
