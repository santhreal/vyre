...
mod tests {
    ...
    /// Pins the launch geometry that decides this kernel's LAUNCH ROUTE.
    ///
    /// Every `GridSync` barrier forces a cooperative launch, which requires
    /// every block co-resident. Exceeding that bound is not an error: the CUDA
    /// driver routes to a host-orchestrated kernel split, where each barrier
    /// becomes a kernel boundary. So the block count decides one cooperative
    /// launch versus six regular ones, which is a cost boundary rather than a
    /// correctness one. Two facts drive it and neither is obvious:
    ///
    /// 1. The dispatch element count is the largest binding, NOT the token
    ///    count. `out_statements` is `2 * num_tokens` words, so the launch is
    ///    `2n` lanes and the block count is `ceil(2n / 256)`, DOUBLE what one
    ///    lane per token would give. That halves every threshold here, which is
    ///    easy to miss.
    /// 2. This kernel uses `Expr::atomic_add` on `out_counts`, so
    ///    `dispatch_element_count_for_program` takes the largest non-shared
    ///    binding branch. `out_statements` is largest either way, so the result
    ///    is the same, but a future edit that shrinks `out_statements` below the
    ///    scratch buffer would change which branch decides and must re-derive
    ///    the numbers below.
    ///
    /// Bug locked out: silently raising `C11_AST_MAX_TOK_SCAN` past the point
    /// where the cooperative launch stops fitting, quietly moving the whole C
    /// frontend onto the six-launch split route with no signal. At the current
    /// 65536 cap the grid is 512 blocks against a cooperative thread-residency
    /// limit of 1020 on a 170 SM Blackwell part, so there is exactly 2x headroom.
    /// If this test starts reporting more than 512 blocks at the cap, that
    /// headroom is being spent and the `c11_statement_bounds` doc comment is
    /// stale.
    #[test]
    fn launch_geometry_stays_within_cooperative_residency_at_the_pipeline_cap() {
        ...
        // The element count is 2n, driven by `out_statements`, once n is past
        // the small-n region where the scratch buffer is the largest binding.
        assert_eq!(
            geometry(256),
            (512, [2, 1, 1]),
        ...
        // The live compiler path. 512 blocks against a 1020 block cooperative
        // limit is the 2x headroom this test exists to defend.
        ...
        // The route transition: 130560 tokens is exactly 1020 blocks, the last
        // grid that still fits one cooperative launch, and the next whole tile
        // past it is the first grid routed to the host-orchestrated split.
        assert_eq!(
    ...
    /// The autotuner must NOT widen this kernel's workgroup.
    ///
    /// `Mode::production_default()` is `NaturalGradient`, so with `VYRE_AUTOTUNER`
    /// unset an eligible 1-D storage-only kernel gets a cold-start workgroup
    /// chosen by the tuner, and on a 1536 thread/SM device it can pick 1024.
    /// That would be the worst width here: `1536 / 1024` is 1 by integer
    /// division, so one block per SM and a third of every SM's thread slots idle,
    /// and because the cooperative residency bound is computed on the EFFECTIVE
    /// width it would drop the cooperative route's ceiling from
    /// `1020 * 256 = 261_120` lanes to `170 * 1024 = 174_080`, lowering the split
    /// crossing point from 130560 tokens to 87040.
    ///
    /// This kernel is exempt for ONE reason, and it is load bearing:
    /// `is_natural_gradient_launch_tunable` requires
    /// `!program.non_composable_with_self`, and this program sets
    /// `with_non_composable_with_self(true)`. That flag exists for self-fusion
    /// safety, NOT for launch geometry, so the exemption is a side effect of an
    /// unrelated property. Dropping the flag would silently re-tune this kernel
    /// to 1024 and move the documented crossing point.
    ///
    /// Note the driver independently agrees this kernel is width-agnostic:
    /// `program_uses_launch_geometry_ids` counts only `Expr::LocalId` and
    /// `Expr::WorkgroupId`, and this kernel uses only `Expr::InvocationId`, the
    /// global lane index, which is invariant under regrouping. So a widened
    /// launch would still compute the right answer; it would only change cost and
    /// the route crossing point.
    #[test]
