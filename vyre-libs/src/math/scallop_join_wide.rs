...
///
/// One lane per relation cell, over the [`vyre_primitives::lane_grid`] owner, so
/// the zero-relation case still yields a launchable grid.
#[must_use]
pub const fn scallop_join_wide_dispatch_grid(_n: u32, _w: u32) -> [u32; 3] {
    vyre_primitives::lane_grid(_n.saturating_mul(_n), SCALLOP_JOIN_WIDE_WORKGROUP_SIZE[0])
}
