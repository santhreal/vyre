...
impl MatchScan for GpuLiteralSet {
    ...
    fn cache_key(&self) -> String {
        ...
        // Mirror `pattern_fingerprint` (literal_set.rs): same slices + the
        // case-insensitive flag, so a ci and a non-ci matcher over identical
        // patterns get DISTINCT cache keys (their prefilter masks differ).
