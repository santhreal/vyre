//! C11 lexer token ids: the single owner of the numbering.
//!
//! The numbering is the wire contract between the host table generator
//! (`vyre-grammar-gen`, which bakes these ids into the DFA and LR blobs) and
//! the GPU lexer and parser that decode those blobs (`vyre-libs::parsing::c`).
//! A value that moves on one side and not the other produces a parser that
//! reads every token as something else without any stage reporting a fault, so
//! the ids live here, in the foundation-layer contract crate that both sides
//! depend down onto, and nowhere else.
//!
...
//!
//! `tests/token_ids_have_one_owner.rs` walks the workspace and fails if any
//! file outside this crate declares a `TOK_`-prefixed constant, so a second
//! copy of any token vocabulary fails on arrival rather than on drift.
