...
use vyre_driver::cache_eviction_heat::{try_entries_to_evict, CacheEntryStats, DECAY_HALF_LIFE_S};
...
fn under_capacity_evicts_nothing() {
    ...
    assert!(try_entries_to_evict(&entries, 10, 300.0)
        .expect("Fix: the fixture entry list must rank without allocation pressure")
        .is_empty());
}
...
fn cold_entry_evicted_before_hot_one() {
    ...
    let evict = try_entries_to_evict(&entries, 1, 300.0)
        .expect("Fix: the fixture entry list must rank without allocation pressure");
...
fn equal_heat_evicts_in_input_order() {
    ...
    let evict = try_entries_to_evict(&entries, 1, 200.0)
        .expect("Fix: the fixture entry list must rank without allocation pressure");
...
fn frequency_dominates_recency_at_equal_age() {
    ...
    let evict = try_entries_to_evict(&entries, 1, 1000.0)
        .expect("Fix: the fixture entry list must rank without allocation pressure");
...
fn recency_dominates_frequency_at_equal_hits() {
    ...
    let evict = try_entries_to_evict(&entries, 1, 3600.0)
        .expect("Fix: the fixture entry list must rank without allocation pressure");
...
fn non_finite_current_time_is_total_and_deterministic() {
    ...
    let evict = try_entries_to_evict(&entries, 1, f64::NAN)
        .expect("Fix: the fixture entry list must rank without allocation pressure");
    assert_eq!(
        evict,
        vec![1, 2],
