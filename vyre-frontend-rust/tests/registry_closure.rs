...
//! other targets in this directory hold it to. It still compiles, still appears
//! in the catalog documents that are generated from source, and still diverges
//! from its reference arm with nothing red. The enumerator that closes this gap
...
///
/// The lexer plan's `build` methods take a `&self` receiver, so the enumeration
/// counts only the free builders the plan delegates to. That population is
/// small: any floor below the measured count would be a floor of zero here, and
/// zero builders are what a broken scan reports, so the floor is the measured
/// count. Adding a builder never fails this; removing one is a decision that has
/// to be recorded on this line.
const BUILDER_FLOOR: usize = 2;
